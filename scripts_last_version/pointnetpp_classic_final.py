...
(NEW v6 - PointNet++):
✅ Backbone reemplazado por PointNet++ (SSG) real:
   - Set Abstraction (FPS + BallQuery) + Feature Propagation
   - Geometría forzada a FP32 incluso con AMP (square_distance, FPS, ball query, interp)
   - BallQuery robusto: no deja idx=N, clampa índices, evita NaNs silenciosos
...
CUDA_VISIBLE_DEVICES=0 python3 pointnetpp_classic_final.py \
  --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
  --out_dir  /home/htaucare/Tesis_Amaro/outputs/pointnetpp/gpu0_run1_neighbors \
...
def normalize_unit_sphere(xyz: torch.Tensor, eps: float = 1e-9) -> torch.Tensor:
    ...
    Normaliza una nube [N,3] a esfera unitaria:
      - centra por media
      - escala por el radio máximo (norma máxima)
    """
...
class NPZDataset(Dataset):
    ...
    usando np.ascontiguousarray + torch.as_tensor.
    """
...
# ============================================================
# (SIGUE) POINTNET++ CORE (FP32 geom robusto) + backbone SSG
# En Parte 2/5: square_distance, index_points, FPS, BallQuery,
# sample_and_group, SetAbstraction, FeaturePropagation, PointNet2Seg.
# ============================================================
...
def _as_fp32(x: torch.Tensor) -> torch.Tensor:
    return x.float() if (x is not None and x.dtype != torch.float32) else x
...
def square_distance(src: torch.Tensor, dst: torch.Tensor) -> torch.Tensor:
    ...
    src: [B, N, 3], dst: [B, M, 3]
    return: [B, N, M] dist^2
    NOTA: forzamos FP32 por estabilidad numérica (aunque afuera haya AMP).
    """
    src = _as_fp32(src)
    dst = _as_fp32(dst)

    # dist = ||src||^2 + ||dst||^2 - 2*src*dst^T
...
def query_ball_point(radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
    ...
    ROBUSTO:
      - setea idx inválidos como N y luego los reemplaza por el primer vecino válido
      - si no hay vecinos, fuerza a 0
      - clamp final
    """
    xyz = _as_fp32(xyz)
...
def sample_and_group(
    ...
    returns:
      new_xyz: [B,npoint,3]
      new_points: [B,npoint,nsample,3+D]  (xyz_rel + feats)
    """
    xyz = _as_fp32(xyz)
...
class PointNetSetAbstraction(nn.Module):
    ...
    Set Abstraction (SSG):
      - FPS -> centers
      - BallQuery -> vecinos
      - MLP (Conv2d 1x1) sobre (xyz_rel + feats)
...
class PointNetFeaturePropagation(nn.Module):
    ...
    Feature Propagation:
      - Interp 3-NN (en FP32)
      - concat con skip (si points1 existe)
      - MLP (Conv1d 1x1)
    """
...
# ============================================================
# POINTNET++ SEGMENTATION MODEL (SSG)
# ============================================================
class PointNet2Seg(nn.Module):
    """
    PointNet++ SSG (tipo "paper-like"):
      SA1: 1024 pts, r=0.10
      SA2:  256 pts, r=0.20
      SA3:   64 pts, r=0.40
      SA4:   16 pts, r=0.80
      FP: upsample de vuelta a N
      head: Conv1d + Dropout + Conv1d -> logits
    """
...
# ============================================================
# (SIGUE) MÉTRICAS + VISUALIZACIÓN + HELPERS (vecinos/trazabilidad)
# En Parte 3/5: métricas macro/d21/vecinos, plots 3D, parse neighbors,
# index discovery/read/sanitize, y run_epoch (v4 comparable train vs val).
# ============================================================
...
def macro_metrics_no_bg(pred: torch.Tensor, gt: torch.Tensor, C: int, bg: int = 0) -> Tuple[float, float]:
    ...
    for c in range(1, int(C)):
        ...
        denom = (tp + fp + fn)
        if denom == 0:
            # clase ausente -> la omitimos del macro (robusto)
            continue
...
def _tooth_metrics_binary(pred: torch.Tensor, gt: torch.Tensor, tooth_idx: int, bg: int = 0) -> Dict[str, float]:
    """
    Métricas binarias por diente (mismo espíritu que d21):
      - sin bg (métrica principal)
      - con bg (bin_acc_all referencia)
    """
...
def eval_neighbors_on_loader(
    ...
    """
    Evalúa neighbors en un loader (promedio por batch).
...
# ============================================================
# (SIGUE) MAIN + logging completo + neighbors + test/plots/infer
# En Parte 4/5: argparse, sanity, model/loss/opt/sched, CSV cols dinámicas,
# history, train loop con prints + neighbors, guardado best/last, y guardado history.json.
# ============================================================

# ============================================================
# MAIN (PointNet++ classic final)
# ============================================================
def _fmt_hms(seconds: float) -> str:
    seconds = float(seconds)
    h = int(seconds // 3600)
...
def main():
    ...
    # métricas train comparables (forward en eval() para TRAIN, sin dropout)
    ...
    # neighbors (igual filosofía PointNet v5 / DGCNN)
    ...
    # =========================================================
    run_meta = {
        "script_name": "pointnetpp_classic_final.py",
    ...
    # =========================================================
    # CSV por epoch (igual estructura que PointNet v5)
    # =========================================================
    ...
    # neighbors history (val por defecto)
    for name, _ in neighbor_list:
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        # 2) métricas train comparables (eval forward)
        ...
        # 4) neighbors (val por epoch, configurable)
        ...
        if do_neighbors:
            ...
            # test por epoch lo dejamos para el final (caro).
        ...
        if neighbor_list:
            for name, _ in neighbor_list:
                history[f"val_{name}_acc"].append(float(nb_vals.get(f"{name}_acc", 0.0)))
                history[f"val_{name}_f1"].append(float(nb_vals.get(f"{name}_f1", 0.0)))
                history[f"val_{name}_iou"].append(float(nb_vals.get(f"{name}_iou", 0.0)))
                history[f"val_{name}_bin_acc_all"].append(float(nb_vals.get(f"{name}_bin_acc_all", 0.0)))
        ...
        # print compacto + neighbors f1 (para no alargar infinito)
        nb_str = ""
        if neighbor_list and do_neighbors:
            parts = [f"{name}_f1={nb_vals.get(f'{name}_f1', 0.0):.3f}" for name, _ in neighbor_list]
    ...
    # =========================================================
    # (SIGUE) TEST(best) + PLOTS + INFERENCIA
    # En Parte 5/5: cargar best/last, run_epoch en test + neighbors test,
    # guardar test_metrics.json, plots Train vs Val (sin testline),
    # y carpeta inference/ con all/errors/d21 (con index discovery).
    # Además: comandos de ejemplo para correr en CUDA:0.
    # =========================================================
    ...
    if bool(args.do_infer):
            ...
            for batch_idx, (xyz, y) in enumerate(loader):
                ...
                for b in range(xyz.shape[0]):
                    ...
                    if index_map and done in index_map:
                        meta_info = index_map[done]
                        safe = _sanitize_tag(meta_info.get("sample_name", ""))
                        if safe:
                            tag += "_" + safe
                    ...
                    manifest_rows.append({
                        "row_i": done,
                        "tag": tag,
                        "sample_name": meta_info.get("sample_name", ""),
                        "jaw": meta_info.get("jaw", ""),
        ...
        if manifest_rows:
            ...
            with open(manifest_path, "w", newline="", encoding="utf-8") as f:
                wcsv = csv.DictWriter(f, fieldnames=manifest_rows[0].keys())
                wcsv.writeheader()
