...
Ejemplo:
python3 pointnet_classic_final.py \
...
def _read_index_csv(path: Path) -> Optional[Dict[int, Dict[str, str]]]:
    ...
    row_i = posición (fila) en el split (df.iloc[row_i]).
    """
...
def main():
    ...
    # Opción A: bg incluido en la loss; lo controlas por peso
    ...
    # loss weights (bg incluido, downweight bg)
    ...
    # logging meta
    run_meta = {
        "script_name": "pointnet_classic_final.py",
    ...
    # Inferencia / visualización + trazabilidad
        ...
        with open(manifest_path, "w", newline="", encoding="utf-8") as fman:
            ...
            with torch.no_grad():
                for r, i in enumerate(idxs, start=1):
                    ...
                    xyz, y = ds_inf[i]  # xyz,y ya vienen normalizados si aplica
                    xyz_b = xyz.unsqueeze(0).to(device, non_blocking=True)
                    logits = model(xyz_b)[0]  # [N,C]
                    pred = logits.argmax(dim=-1).detach().cpu().numpy().astype(np.int32, copy=False)
                    ...
                    title = (
                        ...
                        f"best_epoch={ckpt_epoch} | C={C} | d21={int(args.d21_internal)}"
                    )
