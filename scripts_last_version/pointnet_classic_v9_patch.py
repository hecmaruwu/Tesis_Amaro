...
"""
pointnet_classic_final_v9_patch.py
...
def main():
    ...
    # =========================================================
    run_meta = {
        "script_name": "pointnet_classic_final_v9_patch.py",
    ...
    if args.do_infer:
        ...
        with torch.no_grad():
            for xyz, y, row_i in infer_loader:
                ...
                p_pred = inf_pred / f"{tag}_pred.npy"
                p_gt = inf_pred / f"{tag}_gt.npy"
                p_xyz = inf_pred / f"{tag}_xyz.npy"

                np.save(p_pred, pr_np.astype(np.int64))
                np.save(p_gt, y_np.astype(np.int64))
                np.save(p_xyz, xyz_np.astype(np.float32))
