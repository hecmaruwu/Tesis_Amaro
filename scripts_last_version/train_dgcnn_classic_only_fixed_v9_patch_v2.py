...
"""
train_dgcnn_classic_only_fixed_v9_patch_v2.py

DGCNN (EdgeConv) – Segmentación multiclase dental 3D
(V9 PATCH alineado visual y estructuralmente con pointnet_classic_final_v8_patch.py)

Objetivos:
✅ Mantener la arquitectura DGCNN original (EdgeConv + KNN chunk-safe)
✅ Mantener outputs y trazabilidad del v8 patch
✅ Mantener:
   - history.json
   - history_epoch.jsonl
   - metrics_epoch.csv
...
✅ Igualar el estilo de plots e inferencia visual al PointNet v8 patch
✅ Mantener grad clipping real
✅ Mantener best checkpoint por d21_f1
...
Cambios visuales principales respecto a v8:
1) ✅ Plots train-vs-val con el mismo estilo de PointNet v8
2) ✅ Inferencia 3D con el mismo estilo visual de PointNet v8:
   - GT (todas las clases)
   - Pred (todas las clases)
   - Errores (rojo)
   - Foco d21: TP (verde), FP/FN (rojo)
3) ✅ Manifest de inferencia con columnas:
   - row_i
   - tag
   - sample_name
   - jaw
   ...
   - png_errors
   - png_d21
4) ✅ Mantener todas las mejoras funcionales del v8 patch
...
# ============================================================
# VISUALIZACIÓN (alineada al estilo PointNet v8)
# ============================================================
...
def main():
    ...
    # =========================================================
    run_meta = {
        "script_name": "train_dgcnn_classic_only_fixed_v9_patch_v2.py",
    ...
    if args.infer_only:
        ...
        if "model_state" in ck:
            ...
            raise KeyError(f"Checkpoint sin 'model_state' ni 'model': {best_path}")
