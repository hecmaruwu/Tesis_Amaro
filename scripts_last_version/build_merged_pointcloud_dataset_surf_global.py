...
# -*- coding: utf-8 -*-
"""
Fusión de nubes preprocesadas mediante muestreo global de superficie
en un dataset global PointNet-like.

Entrada:
  processed_flat_safe/<N>/surf_sample_global_upper/<sample>/{point_cloud.npy,labels.npy}
...
def random_split(X, Y, ratios=(0.8, 0.1, 0.1), seed=42):
    """Divide aleatoriamente las muestras en train/val/test según la semilla."""
    seed_all(seed)
    n = X.shape[0]
    idx = np.arange(n)
...
def main():
    ...
    ap.add_argument("--in_root", required=True, help="processed_flat_safe/<N>")
    ap.add_argument("--out_dir", required=True, help="salida merged_<N>")
    ap.add_argument("--n_points", type=int, default=8192)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--jaws", nargs="+", default=["surf_sample_global_upper"],
                help="Lista de jaws a usar (ej: surf_sample_global_upper surf_sample_global_lower)")
    args = ap.parse_args()
    ...
    if not all_X:
        raise RuntimeError(f"No se encontraron muestras. Revisa --in_root y --jaws: {args.jaws}")
    ...
    idx_train, idx_val, idx_test = random_split(X, Y_remap, seed=args.seed)
