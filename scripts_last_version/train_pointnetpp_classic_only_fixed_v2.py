...
"""
train_pointnetpp_classic_only_fixed_v2.py

PointNet++ (SSG) – Segmentación multiclase dental 3D (OPCIÓN A – BG incluido en loss)

FIXES IMPORTANTES:
✅ (FIX) DataLoader/NumPy: usar torch.as_tensor en vez de torch.from_numpy (evita error raro worker)
...
✅ Warmup LR opcional (se implementa en PARTE 2)

Dataset esperado:
  data_dir/X_train.npz, Y_train.npz, X_val.npz, Y_val.npz, X_test.npz, Y_test.npz
  X: [B,N,3], Y: [B,N] con clases internas 0..C-1 (0=bg)

NOTA: Este archivo es PARTE 1/2.
PARTE 2 incluye: main() completo, train loop, logging, plots, test, inferencia con trazabilidad.
"""
...
class NPZDataset(Dataset):
    ...
    def __getitem__(self, i):
        # ✅ FIX: torch.as_tensor evita el bug del worker ("expected np.ndarray ...")
...
def run_epoch(model: nn.Module,
    ...
    """
    (idéntica idea al PointNet classic)
    """
    model.train(train)
...
# ============================================================
# TRAZABILIDAD: index_*.csv discovery + parse (igual que PointNet)
# ============================================================
...
# ============================================================
# VISUALIZACIÓN (se usa en PARTE 2)
# ============================================================
...
# ============================================================
# PARTE 2 continúa con main(): train loop + logging + test + infer
# ============================================================
...
def main():
    ...
    if args.device == "cuda" and torch.cuda.is_available():
        ...
        torch.backends.cudnn.benchmark = True  # RTX3090 friendly
    else:
