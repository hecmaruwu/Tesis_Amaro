...
# -*- coding: utf-8 -*-
"""
plot_selected_qualitative_meshes_v4_oclusal_worst_zoom.py

Versión corregida sin Kaleido ni Chrome.

Objetivo:
- Generar HTML interactivos con Plotly para el análisis cualitativo.
- Mantener las cámaras de la versión anterior en todas las vistas.
- Cambiar SOLO la vista oclusal superior:
    * Mejor caso: z=1.1660883697387594
    * Caso intermedio: z=1.3352015910395147
    * Peor caso: z=1.7262774418516171
...
# Matplotlib queda disponible solo como respaldo opcional.
# No se recomienda para la figura final de mallas, porque puede renderizar
# superficies complejas con artefactos visuales.
import matplotlib
matplotlib.use("Agg")
...
# Cámaras base. Se mantienen los valores de la versión anterior en todas
# las vistas. En oclusal_superior se usa la cámara del mejor caso como base,
# pero más abajo se define una excepción por escena para esa vista.
CAMERA_VIEWS = {
...
# SOLO la vista oclusal superior usa cámaras distintas por fila:
#   scene  = mejor caso
#   scene2 = caso intermedio
#   scene3 = peor caso
...
def apply_zoom_to_camera(camera: Dict[str, Any], zoom: float) -> Dict[str, Any]:
    ...
    En esta versión, si no usa --zoom ni --zoom_by_view, los valores de cámara
    quedan exactamente como fueron definidos arriba.
    """
...
def get_scene_cameras_for_view(
    ...
    """
    Devuelve las cámaras para las tres escenas/subplots.

    Regla general:
        scene, scene2 y scene3 usan la misma cámara.

    Excepción:
        oclusal_superior:
            scene  = mejor caso: z=1.1660883697387594
            scene2 = caso intermedio: z=1.3352015910395147
            scene3 = peor caso: z=1.7262774418516171
    """
...
def build_post_script(sync_cameras: bool) -> str:
    """
    Si --sync_cameras está activo, al mover una escena se sincronizan las tres.
    Por defecto se deja desactivado para respetar la excepción oclusal_superior,
    donde el peor caso usa una cámara distinta.
    """
    if not sync_cameras:
        return """
...
def render_static_matplotlib(
    ...
    for idx, (label, path, mesh) in enumerate(cases, start=1):
        ...
        elev, azim = camera_to_elev_azim(scene_cameras[idx - 1])
...
def main():
    ...
    ap.add_argument(
        "--out_dir",
        default="/home/htaucare/Tesis_Amaro/case_comparisons/qualitative_plotly_mesh_views_v4_oclusal_worst_zoom",
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Sincroniza las cámaras al mover el HTML. Por defecto queda apagado "
            "para respetar que la vista oclusal superior usa una cámara distinta por fila."
        ),
    )
    ...
    run_meta = {
        "script": "plot_selected_qualitative_meshes_v4_oclusal_worst_zoom.py",
    ...
    save_json(run_meta, out_dir / "run_meta_qualitative_plotly_v4_oclusal_worst_zoom.json")
    save_json(cameras, out_dir / "camera_presets_base_v4.json")

    print(f"[OK] Metadata: {out_dir / 'run_meta_qualitative_plotly_v4_oclusal_worst_zoom.json'}")
