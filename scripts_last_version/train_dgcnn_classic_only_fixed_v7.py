...
"""
train_dgcnn_classic_only_fixed_v7.py

DGCNN (EdgeConv) – Segmentación multiclase dental 3D

VERSIÓN v7 (v6 + PLOTS DE VECINOS)
...
NUEVO v7:
✔ Plots de vecinos en out_dir/plots/:
    neighbor_<name>_acc.png
    neighbor_<name>_f1.png
    neighbor_<name>_iou.png
    neighbor_<name>_bin_acc_all.png
  (misma estética que tus plots train vs val)
...
def plot_pointcloud_all_classes(
    ...
    sc1 = ax1.scatter(xyz[:, 0], xyz[:, 1], xyz[:, 2], c=y_gt, s=s, cmap="tab20")
    ax1.set_title("GT")
    _setup_3d_axes(ax1)

    ax2 = fig.add_subplot(1, 2, 2, projection="3d")
    sc2 = ax2.scatter(xyz[:, 0], xyz[:, 1], xyz[:, 2], c=y_pred, s=s, cmap="tab20")
    ax2.set_title("Pred")
    _setup_3d_axes(ax2)
...
def infer_and_save_examples(
    ...
    """
    Carga X_{split}.npz / Y_{split}.npz desde data_dir raíz.
