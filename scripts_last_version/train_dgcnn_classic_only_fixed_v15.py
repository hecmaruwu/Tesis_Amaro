...
"""
train_dgcnn_classic_only_fixed_v15.py
...
FIX v15 (CRÍTICO):
✅ make_loaders() devuelve 4 valores (dl_tr, dl_va, dl_te, ds_te)
✅ main() desempaca 4 valores (ya no rompe con "too many values to unpack")
✅ Mantiene TODO lo demás: outputs, plots, inferencia, vecinos, etc.

Ejemplo (GPU 1):
CUDA_VISIBLE_DEVICES=1 python3 train_dgcnn_classic_only_fixed_v15.py \
  --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
  --out_dir  /home/htaucare/Tesis_Amaro/outputs/dgcnn/gpu1_run1_v15_neighbors \
  ...
  --neighbor_eval_split both --neighbor_every 1 \
  --seed 42 --normalize --plot_every 10 \
  --do_infer --infer_examples 12 --infer_split test --infer_num_workers_cap 2
"""
...
def setup_logging(out_dir: Path, name: str = "dgcnn_v15") -> Logger:
...
def make_loaders(data_dir: Path, bs: int, nw: int, normalize: bool = True):
    ...
    (Para compat con tu stack tipo PointNet v5 y para infer trazable si se necesitara.)
    """
...
# ============================================================
# Neighbors parsing (mantener ambas funciones por compat)
# ============================================================
...
# ============================================================
# Train/Eval helpers (1 epoch) + Train loop (v15)
# - v15: coherente con make_loaders() que devuelve 4:
#         dl_tr, dl_va, dl_te, ds_te
# ============================================================
...
def train_loop_v15(
    ...
    *,
    ...
    history = {
        ...
        "neighbors_test": {},    # llenado en test
    }
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        do_neighbors = (len(neighbors) > 0) and (mode in ("val", "both")) \
        ...
        if do_neighbors:
            ...
            for name, _ in neighbors:
                for m in ["acc", "f1", "iou", "bin_acc_all"]:
                    v = float(nb.get(name, {}).get(m, 0.0))
                    history["neighbors_val"][name][m].append(v)
                    nb_vals_flat[f"{name}_{m}"] = v
        else:
            for name, _ in neighbors:
                for m in ["acc", "f1", "iou", "bin_acc_all"]:
...
# ============================================================
# Inferencia trazable (ALL / ERRORS / D21)
# ============================================================

@torch.no_grad()
def run_inference_v15(
    model: nn.Module,
    dataset: DentalNPZDataset,
    *,
    device: torch.device,
    out_dir: Path,
    C: int,
    ...
    inf_root = Path(out_dir) / "inference"
    ...
    dir_err.mkdir(parents=True, exist_ok=True)
    dir_d21.mkdir(parents=True, exist_ok=True)

    manifest_path = inf_root / "inference_manifest.csv"

    with open(manifest_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["index", "split_id", "error", "d21_present"])

        total = min(len(dataset), int(infer_examples))

        for i in range(total):
            xyz, y = dataset[i]

            xyz_t = torch.as_tensor(xyz, dtype=torch.float32, device=device).unsqueeze(0)
            y_t   = torch.as_tensor(y,   dtype=torch.long,   device=device).unsqueeze(0)

            logits = model(xyz_t, knn_chunk_size=int(knn_chunk_size))
            ...
            # error global
            has_error = bool((pred_np != y_np).any())

            # d21 presente
            d21_present = bool((y_np == int(d21_idx)).any())

            # nombre base
            split_id = f"sample_{i:04d}"
            ...
            plot_3d_seg(
                ...
                pred_np,
                C=C,
                save_path=dir_all / f"{split_id}.png",
                title=split_id,
            )
            ...
            if has_error:
                plot_3d_seg(
                    ...
                    pred_np,
                    C=C,
                    save_path=dir_err / f"{split_id}.png",
                    title=f"{split_id}_error",
                )
            ...
            if d21_present:
                plot_3d_seg(
                    ...
                    pred_np,
                    C=C,
                    save_path=dir_d21 / f"{split_id}.png",
                    title=f"{split_id}_d21",
                )

            writer.writerow([i, split_id, int(has_error), int(d21_present)])

    print(f"[infer] Guardado en {inf_root}")


# ============================================================
# MAIN (v15 FIX DEFINITIVO)
# ============================================================
...
def main():
    ...
    # info dataset
    C = ds_te.Y.shape[-1] if len(ds_te.Y.shape) == 3 else int(ds_te.Y.max() + 1)
    ...
    # -------- LOSS --------
    class_weights = torch.ones(int(C), device=device)
    class_weights[int(bg)] = float(args.bg_weight)
    ...
    # -------- TRAIN --------
    best_epoch, best_val = train_loop_v15(
        model=model,
        dl_tr=dl_tr,
        dl_va=dl_va,
        dl_te=dl_te,
