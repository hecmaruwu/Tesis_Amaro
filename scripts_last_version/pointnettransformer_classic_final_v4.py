...
"""
pointnettransformer_classic_final_v4.py
...
Ejemplo:
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 python3 -u pointnettransformer_classic_final_v4.py \
  --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
  --out_dir  /home/htaucare/Tesis_Amaro/outputs/pointnet_transformer/gpu0_run_fast_v4 \
...
def make_infer_loader(data_dir: Path, split: str, bs: int, nw: int, normalize: bool = True) -> DataLoader:
    ...
    si nw==0). Si quieres forzarlo, deja --infer_num_workers explícito (agregado en v4).
    """
...
class PointNetTransformerSegFast(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor) -> torch.Tensor:
        ...
        #
        # ⚠️ Importante:
        # - hacemos este kNN en FP32 aunque uses AMP, para estabilidad numérica y reproducibilidad.
...
def _format_neighbor_console(metrics: Dict[str, float], neighbor_list: List[Tuple[str, int]]) -> str:
    ...
    for name, _ in neighbor_list:
        ...
        if acc is None and f1 is None and iou is None:
            continue
        # mantenemos 3 decimales como el resto
...
# ============================================================
# ARGPARSE + MAIN COMPLETO (v4)
# ============================================================
def parse_args():
    p = argparse.ArgumentParser(
        description="PointNet-Transformer Segmentation FAST (v4) — mismo esquema outputs que PointNet/DGCNN"
    )
    ...
    p.add_argument("--print_every", type=int, default=1,
                   help="Imprime métricas en consola cada N épocas (default=1)")
    p.add_argument("--save_every", type=int, default=1,
                   help="Guarda history.json incremental + last.pt cada N épocas (default=1)")
    ...
    p.add_argument("--infer_num_workers", type=int, default=None,
                   help="Workers SOLO para infer. Si None, usa min(num_workers, 2) por robustez.")
...
def main():
    ...
    log_line("=== RUN START (v4) ===", run_log, also_print=True)
    ...
    meta = vars(args).copy()
    meta["num_classes"] = C
    meta["script_name"] = "pointnettransformer_classic_final_v4.py"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # TRAIN (no vecinos en train por costo; si quieres, se activa fácil)
        train_metrics = run_epoch(
            ...
            neighbor_list=[],  # train sin vecinos
        )

        # VAL (vecinos cada neighbor_every)
        do_nb = (neighbor_list and (epoch % int(args.neighbor_every) == 0))
        ...
        # guardado incremental por época (JSONL)
        ep_obj = {
            "epoch": int(epoch),
            "lr": float(lr_now),
        ...
        # PRINT (estilo PointNet/DGCNN) + vecinos
        ...
        with open(csv_path, "a", newline="") as f:
            ...
            row = [
                ...
                train_metrics["d21_acc"], val_metrics["d21_acc"],
                train_metrics["d21_f1"], val_metrics["d21_f1"],
                train_metrics["d21_iou"], val_metrics["d21_iou"],
                train_metrics["d21_bin_acc_all"], val_metrics["d21_bin_acc_all"],
                train_metrics["pred_bg_frac"], val_metrics["pred_bg_frac"],
            ]
            for name, _ in neighbor_list:
                row += [
                    val_metrics.get(f"{name}_acc", 0.0),
                    val_metrics.get(f"{name}_f1", 0.0),
                    val_metrics.get(f"{name}_iou", 0.0),
                    val_metrics.get(f"{name}_bin_acc_all", 0.0),
                ]
            writer.writerow(row)
    ...
    # ------------------------------------------------
    # TEST (se hace en PARTE 5 junto a plots+infer)
    # ------------------------------------------------
    # (continúa...)
    ...
    # imprime resumen test (+ vecinos si aplicó)
    nb_str_test = _format_neighbor_console(test_metrics, neighbor_list) if (
        neighbor_list and (args.neighbor_eval_split in ("test", "both"))
    ) else ""
    log_line(
    ...
    # 🔥 FIX importante: aunque history no tenga algo por un corte,
    # igual intentamos los plots si existen las keys.
    ...
    if args.do_infer:
        ...
        # dentro de inference/, dejamos subfolders como tus otros scripts (ordenado)
        ...
        # ⚠️ FIX CLAVE: workers en infer pueden matar matplotlib/IO en algunos entornos.
        # Por defecto, ponemos pocos workers (min(num_workers,2)) o 0 si quieres máxima robustez.
                ...
                for batch in loader:
                    try:
                        ...
                        for b in range(B):
                            ...
                            # plots seguros (si fallan, se loguea pero no corta infer)
...
2) Si tu problema era que NO salían plots/inference:
   fuerza infer con 0 workers (máxima robustez IO/matplotlib):
CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 python3 -u pointnettransformer_classic_final_v4.py \
  --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
  --out_dir  /home/htaucare/Tesis_Amaro/outputs/pointnet_transformer/gpu0_run_fast_v4_infer0w \
  --epochs 120 --batch_size 8 --lr 2e-4 --weight_decay 1e-4 \
  --num_workers 6 --device cuda --use_amp --grad_clip 1.0 \
  --bg_index 0 --bg_weight 0.03 --d21_internal 8 \
  --neighbor_teeth "d11:1,d22:9" --neighbor_every 1 --neighbor_eval_split val \
...
3) Correr en GPU 1:
CUDA_VISIBLE_DEVICES=1 PYTHONUNBUFFERED=1 python3 -u pointnettransformer_classic_final_v4.py \
  --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
  --out_dir  /home/htaucare/Tesis_Amaro/outputs/pointnet_transformer/gpu1_run_fast_v4 \
  --epochs 120 --batch_size 8 --lr 2e-4 --weight_decay 1e-4 \
  --num_workers 6 --device cuda --use_amp --grad_clip 1.0 \
  --bg_index 0 --bg_weight 0.03 --d21_internal 8 \
  --neighbor_teeth "d11:1,d22:9" --neighbor_every 1 --neighbor_eval_split val \
