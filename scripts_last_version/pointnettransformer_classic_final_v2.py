...
"""
pointnettransformer_classic_final_v2.py
...
Ejemplo:
python3 pointnettransformer_classic_final_v2.py \
  --data_dir .../upper_only_surf_global_excl_wisdom_seed42_aug2 \
  --out_dir  .../outputs/pointnettransformer/gpu1_run1_v2 \
...
from collections import defaultdict  # FIX: usado en history
...
def infer_num_classes_from_npz(data_dir: Path) -> int:
    """
    FIX: en v1 se llamaba a _load_label_map / infer_num_classes (no definidos).
    Aquí inferimos C de forma robusta desde Y_train/val/test.
    """
    ys = []
...
def eval_neighbors_on_loader(
    ...
    Mantiene la misma API que la versión v1.
    """
...
class PointNetTransformerSeg(nn.Module):
    ...
    - Para N=8192 puede ser pesado, pero NO eliminamos ninguna característica.
    """
    def __init__(
        self,
        num_classes: int,
...
# ============================================================
# PARTE 5/5 — ARGPARSE + MAIN COMPLETO (CORREGIDO v2)
# ============================================================
def parse_args():
    p = argparse.ArgumentParser(
        description="PointNet-Transformer Segmentation (v2) — mismo esquema outputs que PointNet/DGCNN"
    )
...
def main():
    ...
    # DATASETS (FIX v2)
    # -----------------------
    dl_tr, dl_va, dl_te, _ = make_loaders(
        data_dir=data_dir,
        bs=args.batch_size,
        nw=args.num_workers,
        normalize=True,
