...
"""
train_dgcnn_classic_only_fixed_v9.py

DGCNN (EdgeConv) – Segmentación multiclase dental 3D

VERSIÓN v9 (v8 + FIX: INFER normaliza EXACTAMENTE igual que train/val/test)

Diferencia detectada en v8:
- Train/Val/Test pasan por NPZDataset.__getitem__() y si --normalize => normalize_unit_sphere().
- INFER cargaba X_{split}.npz directo y NO aplicaba normalize_unit_sphere(), aunque hubieras activado --normalize.

Arreglo v9:
✔ infer_and_save_examples(..., normalize=bool) y aplica normalize_unit_sphere() ANTES del forward cuando corresponde.
✔ Con eso, inferencia queda consistente con train/val/test.

Mantiene TODO lo de v8:
✔ Workaround robusto contra NumPy "mezclado" en INFER (sin ufuncs NumPy para máscaras)
✔ Chequeos explícitos de mismatch
...
#   ✅ v9: vecinos listos para imprimir en consola (integración en PARTE 4/5)
# ============================================================
...
def save_run_meta(out_dir: Path, args: argparse.Namespace, extra: Dict[str, Any]):
    ...
    meta = {
        ...
        "script": "train_dgcnn_classic_only_fixed_v9.py",
        "args": vars(args),
        "extra": extra,
    }
...
def main():
    ...
    loss_fn = make_loss_fn(int(C), int(bg), float(args.bg_weight), device)
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        tr = run_epoch(model, dl_tr, optim, loss_fn, C, d21_idx, device, bg, True, bool(args.use_amp), float(args.grad_clip))
        va = run_epoch(model, dl_va, None, loss_fn, C, d21_idx, device, bg, False, bool(args.use_amp), None)

        neigh_val = {}
        if neighbors:
            neigh_val = eval_neighbors_on_loader(model, dl_va, device, neighbors, bg, bool(args.use_amp))

        scheduler.step()
        lr = get_lr(optim)

        for k in ("loss","f1_macro","iou_macro","acc_all","acc_no_bg","d21_acc","d21_f1","d21_iou","d21_bin_acc_all","pred_bg_frac"):
        ...
        if neighbors:
            ...
            for name, _ in neighbors:
                neighbor_parts.append(
                    f"{name}: acc={neigh_val.get(f'{name}_acc',0):.3f} "
                    f"f1={neigh_val.get(f'{name}_f1',0):.3f} "
                    f"iou={neigh_val.get(f'{name}_iou',0):.3f}"
                )
            neighbor_str = " | neighbors(val) -> " + " | ".join(neighbor_parts)
        ...
        if va["iou_macro"] > best_val:
            best_val = float(va["iou_macro"])
            best_epoch = epoch
            torch.save({"model_state": model.state_dict()}, out_dir / "best.pt")

        torch.save({"model_state": model.state_dict()}, out_dir / "last.pt")
    ...
    te = run_epoch(model, dl_te, None, loss_fn, C, d21_idx, device, bg, False, bool(args.use_amp), None)
    save_test_metrics(out_dir, te)

    # INFER (FIX v9: ahora pasa normalize)
    if bool(args.do_infer):
        infer_and_save_examples(
            ...
            normalize=bool(args.normalize),   # 🔥 FIX CLAVE v9
        )

# ============================================================
# PARTE 5/5 — INFERENCIA COMPLETA + PLOTS 3D + CIERRE (v9)
# ============================================================
...
# ============================================================
# INFERENCIA CON TRAZABILIDAD (v9)
#   ✅ FIX: aplica normalize_unit_sphere igual que el Dataset si normalize=True
#   ✅ Manifest CSV para trazabilidad (extra, no rompe outputs previos)
# ============================================================

def _maybe_normalize_infer(xyz_np: np.ndarray, normalize: bool) -> np.ndarray:
    """
    Replica EXACTO el comportamiento del Dataset:
      - convierte a torch
      - normalize_unit_sphere
      - vuelve a numpy float32
    """
    if not bool(normalize):
        return xyz_np.astype(np.float32, copy=False)

    t = torch.as_tensor(xyz_np, dtype=torch.float32)
    t = normalize_unit_sphere(t)
...
def infer_and_save_examples(
    ...
    manifest_path = out_inf / "inference_manifest.csv"
    manifest_exists = manifest_path.exists()
    with manifest_path.open("a", newline="", encoding="utf-8") as fman:
        fieldnames = [
            "i", "split", "trace_label",
            "png_all", "png_err", "png_d21",
            "normalize",
        ]
        wman = csv.DictWriter(fman, fieldnames=fieldnames)
        ...
        with torch.no_grad():
            for i in range(K):
                xyz_raw = X[i].astype(np.float32, copy=False)
                ygt = Y[i].astype(np.int32, copy=False)

                # ✅ FIX v9: normaliza igual que entrenamiento si args.normalize=True
                xyz = _maybe_normalize_infer(xyz_raw, normalize=bool(normalize))

                label = f"i={i:04d}"
                trace_label = ""
                if idx_map is not None and i in idx_map:
                    trace_label = _pick_trace_label(idx_map[i])
                    label = f"i={i:04d} | {trace_label}"
                ...
                p_all = base.with_suffix(".all.png")
                p_err = base.with_suffix(".err.png")
                p_d21 = base.with_suffix(".d21.png")
                ...
                wman.writerow({
                    ...
                    "png_all": str(p_all.name),
                    "png_err": str(p_err.name),
                    "png_d21": str(p_d21.name),
...
# ------------------------------------------------------------
# EJEMPLOS (COMENTADOS) DE CÓMO CORRER ESTE SCRIPT (v9)
# ------------------------------------------------------------
# Asumiendo:
#   - Estás en: /home/htaucare/Tesis_Amaro/scripts_last_version
#   - El script se llama: train_dgcnn_classic_only_fixed_v9.py
...
#   CUDA_VISIBLE_DEVICES=0 python3 -u train_dgcnn_classic_only_fixed_v9.py \
#     --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
#     --out_dir  /home/htaucare/Tesis_Amaro/outputs/dgcnn/gpu0_v9_balanced \
...
#   CUDA_VISIBLE_DEVICES=0 python3 -u train_dgcnn_classic_only_fixed_v9.py \
#     --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
#     --out_dir  /home/htaucare/Tesis_Amaro/outputs/dgcnn/gpu0_v9_ultrasafe_bs12 \
...
#   CUDA_VISIBLE_DEVICES=0 python3 -u train_dgcnn_classic_only_fixed_v9.py \
#     --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
#     --out_dir  /home/htaucare/Tesis_Amaro/outputs/dgcnn/gpu0_v9_max_quality \
#     --epochs 120 --batch_size 16 --lr 2e-4 --weight_decay 1e-4 --dropout 0.5 \
#     --num_workers 6 --device cuda --use_amp --grad_clip 1.0 \
...
#   CUDA_VISIBLE_DEVICES=0 python3 -u train_dgcnn_classic_only_fixed_v9.py \
#     --data_dir /home/htaucare/Tesis_Amaro/data/Teeth_3ds/fixed_split/8192/upper_only_surf_global_excl_wisdom_seed42_aug2 \
#     --out_dir  /home/htaucare/Tesis_Amaro/outputs/dgcnn/gpu0_v9_noinfer \
#     --epochs 120 --batch_size 16 --lr 2e-4 --weight_decay 1e-4 --dropout 0.5 \
#     --num_workers 6 --device cuda --use_amp --grad_clip 1.0 \
#     --k 20 --emb_dims 768 --knn_chunk_size 256 \
#     --bg_class 0 --bg_weight 0.03 --d21_internal 8 \
