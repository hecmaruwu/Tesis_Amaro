...
"""
pointnettransformer_classic_final_v1.py
...
Ejemplo (cuando esté completo):
python3 pointnettransformer_classic_final_v1.py \
  --data_dir .../upper_only_surf_global_excl_wisdom_seed42_aug2 \
  --out_dir  .../outputs/pointnettransformer/gpu1_run1_v1 \
  --epochs 120 --batch_size 16 --lr 2e-4 --weight_decay 1e-4 \
...
class PointNetTransformerSeg(nn.Module):
    """
    Segmentación punto-a-punto:
      xyz -> embed -> (pos enc opcional) -> TransformerEncoder -> head -> logits [B,N,C]

    Nota:
    - Sin máscara (todos los puntos se atienden entre sí) => O(N^2).
    - Para N=8192, esto puede ser MUY pesado. Por eso el script permitirá
      usar "token_drop"/subsample en forward (lo agregamos en MAIN, parte 4/5),
      o usar d_model/chunking moderado.
    """
    def __init__(
        self,
        num_classes: int,
        ...
        self.embed = PointMLPEmbed(d_model=int(d_model), hidden=int(embed_hidden), dropout=float(dropout))
        self.pos = PositionalEncodingMLP(d_model=int(d_model), hidden=int(pos_hidden), dropout=float(dropout)) if self.use_pos_mlp else None
...
# ============================================================
# TRAIN / EVAL (mismo estilo v4) + VECINOS (DGCNN-like)
# ============================================================
@torch.no_grad()
...
def _neighbors_metrics_from_logits(
    ...
    """
    Métricas binarias para dientes vecinos (estilo DGCNN):
...
def run_epoch(
    ...
    (NEW) Vecinos:
      - Si neighbor_list != None: agrega métricas promedio por batch para vecinos.
    """
    ...
    # sums vecinos (promedio batch->epoch)
    ...
    # std vecinos si se pide
    nb_batch_stats: Dict[str, List[float]] = {}
    if collect_batch_stats and neighbor_list:
        for name, _ in neighbor_list:
            for k in ("acc", "f1", "iou", "bin_acc_all"):
                nb_batch_stats[f"{name}_{k}"] = []
    ...
    for xyz, y in loader:
        ...
        if train:
            ...
            nb_metrics = _neighbors_metrics_from_logits(logits_eval, y, neighbor_list=neighbor_list, bg=bg)
            ...
            with torch.no_grad():
                ...
                nb_metrics = _neighbors_metrics_from_logits(logits, y, neighbor_list=neighbor_list, bg=bg)
        ...
        if collect_batch_stats:
            ...
            if neighbor_list:
                for name, _ in neighbor_list:
                    nb_batch_stats[f"{name}_acc"].append(float(nb_metrics.get(f"{name}_acc", 0.0)))
                    nb_batch_stats[f"{name}_f1"].append(float(nb_metrics.get(f"{name}_f1", 0.0)))
                    nb_batch_stats[f"{name}_iou"].append(float(nb_metrics.get(f"{name}_iou", 0.0)))
                    nb_batch_stats[f"{name}_bin_acc_all"].append(float(nb_metrics.get(f"{name}_bin_acc_all", 0.0)))
    ...
    if collect_batch_stats:
        ...
        if neighbor_list:
            for k, v in nb_batch_stats.items():
                _, std = _mean_std(v)
...
# ============================================================
# PARTE 4/5 — ARGPARSE + SETUP + TRAIN LOOP (con vecinos)
# ============================================================
def parse_args():
    p = argparse.ArgumentParser(
        description="PointNet-Transformer Segmentation (v1) — mismo esquema outputs que PointNet/DGCNN"
    )
...
def main():
    ...
    # DATASETS
    # -----------------------
    ds_tr = NPZPointDataset(data_dir, "train")
    ds_va = NPZPointDataset(data_dir, "val")
    ds_te = NPZPointDataset(data_dir, "test")
    ...
    # -----------------------
    neighbor_list: List[Tuple[str, int]] = []
    if args.neighbor_teeth:
        parts = [x.strip() for x in args.neighbor_teeth.split(",") if x.strip()]
        for p in parts:
            name, idx = p.split(":")
            neighbor_list.append((name.strip(), int(idx)))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        msg = (
            ...
            f"ioum={train_metrics['iou_macro']:.3f} | "
            f"val loss={val_metrics['loss']:.4f} "
            f"f1m={val_metrics['f1_macro']:.3f} "
            f"ioum={val_metrics['iou_macro']:.3f} "
        ...
        if neighbor_list:
            for name, _ in neighbor_list:
                ...
                if key in val_metrics:
                    msg += f" | {name} f1={val_metrics[key]:.3f}"
    ...
    print(
        ...
        f"d21 acc={test_metrics['d21_acc']:.3f} "
    ...
    if neighbor_list:
        ...
        for name, _ in neighbor_list:
            if f"{name}_f1" in test_metrics:
                print(
                    f"  {name}: "
                    f"f1={test_metrics[f'{name}_f1']:.3f} "
                    f"iou={test_metrics[f'{name}_iou']:.3f}"
                )
    ...
    if args.do_infer:
        ...
        loader = dl_te if args.infer_split == "test" else dl_va
        model.eval()
        ...
        with torch.no_grad():
            for xyz, y in loader:
                ...
                for b in range(xyz.shape[0]):
                    ...
                    tag = f"{args.infer_split}_sample_{done}"
