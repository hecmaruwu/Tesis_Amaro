...
✅ Matplotlib/may_share_memory crash: convierte colores a float64 y xyz a float64 antes de scatter

(NEW v2):
✅ Plots Train vs Test (línea train por época + test como línea horizontal + best_epoch como línea vertical)
✅ test_metrics.json incluye desviación estándar (std) SOLO para TEST (por batch) para métricas clave
...
Ejemplo:
python3 pointnet_classic_final_v2.py \
...
# (NEW v2) helper: pretty time format
...
def make_loaders(data_dir: Path, bs: int, nw: int, normalize: bool = True):
    ...
    # Nota: prefetch_factor sólo aplica si num_workers>0
    common = dict(
        batch_size=int(bs),
        num_workers=int(nw),
        pin_memory=True,
...
def d21_metrics_binary(
    ...
    include_bg=False => excluye puntos bg del cálculo (tu métrica principal clínica)
    include_bg=True  => incluye bg también (referencia; tiende a inflar acc si hay mucho bg)
    """
    pred = pred.reshape(-1)
    gt = gt.reshape(-1)
...
# (NEW v2) helper: std robusto desde lista (por batch)
...
# ============================================================
# TRAIN / EVAL (AMP moderno + métricas)  --- MOD v2
# ============================================================
@torch.no_grad()
...
def run_epoch(
    ...
    collect_batch_stats: bool = False  # NEW v2
) -> Dict[str, float]:
    model.train(train)
    ...
    # NEW v2: recolectar métricas por batch (solo se usará en test)
    ...
    # NEW v2: si recolectamos stats (solo test), añadimos std
    if collect_batch_stats:
        out_std = {}
...
def plot_train_vs_test(
    ...
    plt.plot(epochs, train_curve, label="train", linewidth=2)

    # línea horizontal test
    plt.axhline(y=test_value, linestyle="--", linewidth=2, label="test(best)")

    # línea vertical best_epoch
    if best_epoch > 0:
        plt.axvline(x=best_epoch, linestyle=":", linewidth=2, label="best_epoch")
    ...
    plt.close()
# ============================================================
# MAIN (script final v2)  ✅ PRINT COMPLETO + PLOTS LOSS + VECINOS
# ============================================================
def main():
    ...
    def _resolve_tooth_internal(target_fdi: int) -> Optional[int]:
        """
    ...
    def plot_train_val_with_testline(name: str, y_tr: List[float], y_va: List[float], y_test: float, out_png: Path, best_epoch: Optional[int] = None):
        out_png.parent.mkdir(parents=True, exist_ok=True)
        plt.figure(figsize=(7, 4))
        plt.plot(y_tr, label="train")
        plt.plot(y_va, label="val")
        plt.axhline(float(y_test), linestyle="--", label="test(best)")
        if best_epoch is not None and best_epoch > 0:
            plt.axvline(best_epoch - 1, linestyle=":", label=f"best_epoch={best_epoch}")
        plt.xlabel("epoch")
        plt.ylabel(name)
        plt.title(f"{name} (Train/Val + Test line)")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_png, dpi=250)
        plt.close()
    ...
    # =========================================================
    # Resolver vecinos del 21 (FDI: 11 y 22 por defecto)
    # =========================================================
    d21_int = int(args.d21_internal)

    d11_int = _resolve_tooth_internal(11)
    d22_int = _resolve_tooth_internal(22)

    # fallback típico (tu set: [11..17]=1..7 y [21..27]=8..14 si d21=8)
    if d11_int is None:
    ...
    print(f"[INFO] d21_internal={d21_int} | neighbors=" + ", ".join([f"{k}={v}" for k, v in neighbor_list]) + ("" if neighbor_list else " (none)"))

    # =========================================================
    # LOGGING FILES
    # =========================================================
    run_meta = {
        "script_name": "pointnet_classic_final_v2.py",
    ...
    # history completo (train + val) + vecinos
    history: Dict[str, List[float]] = {}
    def _mk(key: str): history[key] = []

    # core
    for k in ("loss", "acc_all", "acc_no_bg", "f1_macro", "iou_macro", "d21_acc", "d21_f1", "d21_iou", "d21_bin_acc_all", "pred_bg_frac"):
        _mk(f"train_{k}")
        _mk(f"val_{k}")
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        tr = run_epoch(
            ...
            d21_idx=d21_int, device=device, bg=bg, train=True,
        ...
        # vecinos: los calculamos rápido desde preds/gt? (run_epoch no los devuelve).
        # Para no re-forward completo, hacemos un mini-eval global por batches SOLO para vecinos (barato).
        # Si quieres 0 overhead, se puede integrar dentro de run_epoch, pero aquí lo dejamos en main.

        def _eval_neighbors_on_loader(loader: DataLoader) -> Dict[str, float]:
        ...
        tr_nei = _eval_neighbors_on_loader(dl_tr)
        va_nei = _eval_neighbors_on_loader(dl_va)
        ...
        # PRINT COMPLETO estilo antiguo
        nei_str = ""
        if neighbor_list:
            parts = []
            for name, _idx in neighbor_list:
                parts.append(f"{name} f1={va_nei.get(f'{name}_f1',0.0):.3f} iou={va_nei.get(f'{name}_iou',0.0):.3f}")
            nei_str = " | " + " | ".join(parts)
    ...
    # =========================================================
    # TEST con best (y std si run_epoch lo soporta)
    # =========================================================
    ckpt = torch.load(best_path, map_location=device)
    best_epoch = int(ckpt.get("epoch", best_epoch if best_epoch > 0 else -1))
    model.load_state_dict(ckpt["model"])

    # Si tu run_epoch soporta collect_batch_stats, úsalo; si no, cae a normal
    try:
        te = run_epoch(
    ...
    if neighbor_list:
        ...
        with torch.no_grad():
            for xyz, y in dl_te:
                ...
                nb += 1
        nb = max(1, nb)
        te_nei = {k: v / nb for k, v in sums.items()}
    ...
    # =========================================================
    # PLOTS (Train vs Val) + (opcional) línea test
    # =========================================================
    plots_dir = out_dir / "plots"
    plots_dir.mkdir(parents=True, exist_ok=True)

    # loss + core
    plot_train_val("loss", history["train_loss"], history["val_loss"], plots_dir / "loss_train_val.png", best_epoch=best_epoch)
    plot_train_val("f1_macro_no_bg", history["train_f1_macro"], history["val_f1_macro"], plots_dir / "f1_macro_train_val.png", best_epoch=best_epoch)
    plot_train_val("iou_macro_no_bg", history["train_iou_macro"], history["val_iou_macro"], plots_dir / "iou_macro_train_val.png", best_epoch=best_epoch)
    plot_train_val("acc_all", history["train_acc_all"], history["val_acc_all"], plots_dir / "acc_all_train_val.png", best_epoch=best_epoch)
    plot_train_val("acc_no_bg", history["train_acc_no_bg"], history["val_acc_no_bg"], plots_dir / "acc_no_bg_train_val.png", best_epoch=best_epoch)
    plot_train_val("pred_bg_frac", history["train_pred_bg_frac"], history["val_pred_bg_frac"], plots_dir / "pred_bg_frac_train_val.png", best_epoch=best_epoch)

    # d21 + vecinos (aquí sí tiene sentido poner una línea test como referencia visual)
    plot_train_val_with_testline("d21_f1", history["train_d21_f1"], history["val_d21_f1"], float(te.get("d21_f1", 0.0)),
                                 plots_dir / "d21_f1_train_val_testline.png", best_epoch=best_epoch)
    plot_train_val_with_testline("d21_acc", history["train_d21_acc"], history["val_d21_acc"], float(te.get("d21_acc", 0.0)),
                                 plots_dir / "d21_acc_train_val_testline.png", best_epoch=best_epoch)
    plot_train_val_with_testline("d21_iou", history["train_d21_iou"], history["val_d21_iou"], float(te.get("d21_iou", 0.0)),
                                 plots_dir / "d21_iou_train_val_testline.png", best_epoch=best_epoch)
    plot_train_val_with_testline("d21_bin_acc_all", history["train_d21_bin_acc_all"], history["val_d21_bin_acc_all"], float(te.get("d21_bin_acc_all", 0.0)),
                                 plots_dir / "d21_bin_acc_all_train_val_testline.png", best_epoch=best_epoch)

    for name, _idx in neighbor_list:
        # si no quieres test-line para vecinos, cambia a plot_train_val
        plot_train_val_with_testline(f"{name}_f1", history[f"train_{name}_f1"], history[f"val_{name}_f1"],
                                     float(te_nei.get(f"{name}_f1", 0.0)), plots_dir / f"{name}_f1_train_val_testline.png", best_epoch=best_epoch)
        plot_train_val_with_testline(f"{name}_iou", history[f"train_{name}_iou"], history[f"val_{name}_iou"],
                                     float(te_nei.get(f"{name}_iou", 0.0)), plots_dir / f"{name}_iou_train_val_testline.png", best_epoch=best_epoch)
        plot_train_val_with_testline(f"{name}_acc", history[f"train_{name}_acc"], history[f"val_{name}_acc"],
                                     float(te_nei.get(f"{name}_acc", 0.0)), plots_dir / f"{name}_acc_train_val_testline.png", best_epoch=best_epoch)

    # =========================================================
    # Inferencia (si ya la tenías en tu script, déjala debajo tal cual)
    # =========================================================
    # >>> aquí puedes pegar tu bloque existente de inferencia (index_*.csv, manifest, etc.)
    #     (no lo repito para no romper lo que ya tienes funcionando)
    # ---------------------------------------------------------

    print(
        f"[DONE] out_dir={out_dir} | total_sec={total_sec:.1f} ({_fmt_hms(total_sec)}) | "
        f"best_epoch={best_epoch} | best_val_f1_macro(no_bg)={best_val_f1:.4f} | d21_f1_test={float(te.get('d21_f1',0.0)):.4f}"
    )


    # =========================================================
    # INFERENCIA / VISUALIZACIÓN + TRAZABILIDAD (index_*.csv)
    # =========================================================
    if args.do_infer and int(args.infer_examples) > 0:
        ...
        if split == "test":
            ds_inf = ds_te  # ya lo tienes desde make_loaders
        elif split == "val":
            ds_inf = NPZDataset(
                data_dir / "X_val.npz",
                data_dir / "Y_val.npz",
        ...
        # --------- trazabilidad: buscar index_{split}.csv ----------
        ...
        with open(manifest_path, "w", newline="", encoding="utf-8") as fman:
            ...
            with torch.no_grad():
                for r, i in enumerate(idxs, start=1):
                    ...
                    # xyz,y normalizados si aplica
                    xyz, y = ds_inf[i]                   # xyz: [N,3], y: [N]
                    xyz_b = xyz.unsqueeze(0).to(device, non_blocking=True)  # [1,N,3]
                    ...
                    # metadata desde index csv si existe
                    ...
                    # best_epoch: usa best_epoch si existe, si no ckpt.get('epoch')
