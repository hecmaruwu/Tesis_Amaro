...
Crea un split nuevo con:
 ...
 - (opcional) límite de fracción máxima para fondo (label 0 ORIGINAL) en la nube.

Soporta datasets apilados (X_train/val/test.npz; Y_train/val/test.npz) o archivos por muestra.
Salida en .npz: out_split/X_{split}.npz, out_split/Y_{split}.npz más artifacts/label_map.json.
"""
...
def list_label_files(split_dir: Path):
    # Caso A: X_train.npz/Y_train.npz ...
    stacked = []
...
def process_split(in_split: Path, out_split: Path, N_target, cap_bg_frac, seed, bg_original_ids=(0,)):
    ...
    if mode == "A":
        for xf, yf, splitname in stacked_info:
            X = _load_array(xf).astype(np.float32)
            Y = _load_array(yf).astype(np.int64)
        ...
        Xo = np.stack(X_list, axis=0).astype(np.float32)
        Yo = np.stack(Y_list, axis=0).astype(np.int32)
        save_npz(out_split/"X_train.npz", "X", Xo)
...
def main():
    ...
    ap.add_argument("--in_split", required=True, help="Ej: data/Teeth_3ds/merged_dataset")
    ap.add_argument("--out_split", required=True, help="Ej: data/Teeth_3ds/fixednp_dataset")
    ap.add_argument("--N", default="auto", help="N puntos fijos (o 'auto' = mínimo posible)")
    ap.add_argument("--cap_bg", type=float, default=None, help="Fracción máxima de fondo (label 0 original)")
