...
# -*- coding: utf-8 -*-
"""
Augmentación + Split + Remapeo GLOBAL + Balanceo de clases + Control de fondo.

Versión robusta con:
 - Remapeo global 0..C-1
 - Aumento de datos configurable
 - Submuestreo fijo (N)
 - Cálculo automático de class_weights
 - Validación EDA detallada

Compatible con train_models_v7/v8.py
"""
...
def compute_class_weights(Y_train, ignore_label=0):
    """Calcula pesos inversos a la frecuencia (ignorando fondo)."""
    flat = np.concatenate([y.ravel() for y in Y_train])
    counts = Counter(flat)
    ...
    for cls, cnt in counts.items():
        if cls == ignore_label:
            continue
        weights[int(cls)] = float(total / (len(counts) * cnt))
    return weights
...
def main():
    ...
    print(f"[MAP] Global remap: {len(id2idx)} clases totales (0..{len(id2idx)-1})")
    ...
    cw = compute_class_weights([splits["train"][1]])
    cw_json = {int(k): float(v) for k, v in cw.items()}

    np.save(out / "artifacts" / "class_weights.npy", cw_json)
    json.dump(cw_json, open(out / "artifacts" / "class_weights.json", "w"), indent=2)
    print(f"[BAL] Class weights guardados (sin fondo): {len(cw_json)} clases")
    ...
    print(f"\n[DONE] Dataset listo en: {out}")
    print(f"[META] {len(id2idx)} clases totales, class_weights.json guardado.")
