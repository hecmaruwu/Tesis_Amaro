...
- ReduceLROnPlateau compatible con PyTorch antiguos (sin verbose)
"""
...
def normalize_cloud(x):
    ...
    x = x - centroid
    furthest = torch.sqrt((x**2).sum(dim=-1))        # (B,P)
    furthest = furthest.max(dim=1, keepdim=True)[0]  # (B,1)
...
# Clase 0 = Encía; luego dientes 11..48 (32 dientes)
...
class PointNetSeg(nn.Module):
    ...
    def forward(self, xyz):  # (B,P,3)
        ...
        # como en el paper: concatenamos global con features intermedias
        x_cat = torch.cat([xg, x2], 1)                # (B,1024+128,P)
...
class SA_Layer(nn.Module):
    ...
    def forward(self, xyz, feats):
        ...
        Midx = torch.arange(M, device=xyz.device)[None,:,None]
        neigh_xyz = xyz[Bidx.squeeze(-1), knn]              # (B,M,K,3)
        neigh_xyz = neigh_xyz.permute(0,1,3,2)              # (B,M,3,K)

        if feats is not None:
            f2 = feats.transpose(1,2)                       # (B,P,C)
            neigh_f = f2[Bidx.squeeze(-1), knn]             # (B,M,K,C)
        ...
        cat_flat = cat.reshape(Bm*Mm, Cm, K)                 # (B*M, C_in, K)
...
def one_epoch(model, loader, optimizer, criterion, device, metrics_bundle: MetricsBundle=None):
    ...
    for xb, yb in loader:
        ...
        xb = normalize_cloud(xb)                 # (B,P,3) — todos los modelos aceptan este formato
...
def train_model(model_name: str, loaders, num_classes: int, args, device, base_outdir: Path):
    ...
    for ep in range(1, args.epochs+1):
        ...
        # Log consola (estilo que vienes usando)
