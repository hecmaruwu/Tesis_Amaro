...
# -*- coding: utf-8 -*-
"""
train_models_v2.py (v1.4 - Final estable)

Modelos:
  - PointNet (Qi et al., 2017)
  - PointNet++ (SSG jerárquico con k-NN estable)
  - DilatedToothSegNet (Kre et al., 2023)
  - Transformer 3D (Point Transformer simplificado)

Consistente con el capítulo metodológico:
- T-Net sin regularizador ortogonal
- PointNet++ con k-NN en lugar de FPS/Ball Query
- Transformer sin positional encoding (nubes alineadas)
"""
...
class MetricsBundle:
    ...
    def __init__(self,num_classes:int,device:torch.device):
        ...
        self.reset_cm()
    def reset_cm(self): self.cm=torch.zeros((self.num_classes,self.num_classes),
                                           device=self.device,dtype=torch.long)
    @torch.no_grad()
    def update(self,logits,y_true):
        preds=logits.argmax(dim=-1)
        t=y_true.view(-1); p=preds.view(-1)
        valid=(t>=0)&(t<self.num_classes); t=t[valid]; p=p[valid]
...
class PointNetSeg(nn.Module):
    ...
    def forward(self,xyz):
        ...
        x=self.dropout(x); return self.fconv3(x).transpose(2,1)


# ===========================================================
# ------------- PointNet++ (SSG con k-NN estable) -----------
# ===========================================================
...
def three_nn_interp(xyz1, xyz2, feats2, k=3):
    ...
    # recolectar feats2 en (B, N2, C2) -> usar batched_gather
...
class SA_Layer(nn.Module):
    """
    Set Abstraction con:
     - muestreo de centros estratificado (stride/4)
     - vecindad k-NN
    ...
    def forward(self, xyz, feats):
        ...
        xyz:   (B, P, 3)
        feats: (B, C, P) o None
        return: (centers_xyz[B,M,3], out_feats[B,out_ch,M])
        """
        B, P, _ = xyz.shape
        M = max(1, P // 4)

        # centros por muestreo uniforme de índices
