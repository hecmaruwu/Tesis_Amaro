...
- Usa class weights balanceados.
- Limpieza de GPU entre modelos.
...
def compute_class_weights_from_train(Y_train: np.ndarray, num_classes: int, ignore_index: int = 0, smooth: float = 1.0):
    """
    Ponderación inversa a la frecuencia por clase en TRAIN (suavizada).
    - Devuelve vector de tamaño C; peso[ignore_index]=0 para que no afecte.
    """
    flat = Y_train.ravel()
    counts = np.bincount(flat, minlength=num_classes).astype(np.float64)
    counts[ignore_index] = 0.0  # no queremos que el fondo influya
    # evitar ceros
    counts = counts + smooth
    inv = 1.0 / counts
    inv[ignore_index] = 0.0
    w = inv / inv.max()  # normaliza a [0,1]
...
def one_epoch(model, loader, optimizer, criterion, device, metrics_bundle: MetricsBundle=None,
              amp: bool = True, clip_grad: float = 1.0, scaler: torch.cuda.amp.GradScaler | None = None):
    ...
    for xb, yb in loader:
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                ...
                print("[WARN] OOM: batch saltado para liberar memoria.")
                continue
            raise
...
def evaluate_detailed_per_class(cm_bundle: MetricsBundle, num_classes: int, class_names=None):
    cm = cm_bundle.cm.float()
    tp = torch.diag(cm)
    gt = cm.sum(1)
    pd = cm.sum(0)
    ...
    for i in range(num_classes):
        per_class.append({
            "class_id": i,
            "class_name": (class_names[i] if (class_names is not None and i < len(class_names)) else f"class_{i}"),
    ...
    if class_names is not None:
        try:
            idx=class_names.index("Diente 21")
            if idx < num_classes: d21_metrics=per_class[idx]
        except ValueError:
            pass
    return oa, per_class, d21_metrics

def train_model(model_name: str, loaders, num_classes: int, args, device, base_outdir: Path, class_weights: torch.Tensor | None):
    ...
    for ep in range(1, args.epochs+1):
        ...
        tr_loss, tr_macro = one_epoch(model, loaders["train"], optimizer, criterion, device, mb_tr,
                                      amp=args.amp, clip_grad=args.clip_grad, scaler=scaler)
    ...
    mb_test_full = MetricsBundle(num_classes, device, ignore_index=0)
    ...
    oa, per_class, d21 = evaluate_detailed_per_class(mb_test_full, num_classes, class_names=CLASS_NAMES)
    ...
    summary_txt = (
        ...
        f"AMP: {args.amp}  clip_grad: {args.clip_grad}\n"
    ...
    # liberar memoria del modelo antes de pasar al siguiente
    result = {
        "run_dir": str(run_dir),
        "model": model_name,
        "timestamp": timestamp,
...
def main():
    ...
    # EarlyStopping
    ap.add_argument("--es_patience", type=int, default=35, help="Paciencia para early stopping (val_loss).")
    ap.add_argument("--es_delta", type=float, default=1e-4, help="Mejora mínima para resetear paciencia.")

    # Entrenamiento estable
    ap.add_argument("--amp", action="store_true", help="Mixed precision (float16) si hay CUDA")
    ...
    class_weights = compute_class_weights_from_train(Ytr, num_classes, ignore_index=0, smooth=1.0)
    print(f"[WEIGHTS] ejemplo primeras 10: {class_weights[:10].tolist()}")
    ...
    rows = []
    for m in models:
        res = train_model(m, loaders, num_classes, args, device, base_outdir, class_weights)
        rows.append(res)
