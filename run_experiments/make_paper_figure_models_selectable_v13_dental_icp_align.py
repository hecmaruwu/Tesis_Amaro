...
"""
make_paper_figure_models_selectable_v13_dental_icp_align.py

Versión v13:
- Mantiene las mismas vistas/salidas de v12:
    PNG resumen 2x3
    HTML Plotly filtrable
    PNG individuales
    summary.json
- Cambia SOLO la forma de alinear:
    1) ancla dental de malla
    2) foreground de nube
    3) bbox/PCA coarse
    4) ICP rígido/similarity restringido a zona dental
- Mantiene compatibilidad con align_mode antiguos:
    none
    robust_bbox
    dental_bbox
    dental_pca
...
IMPORTANTE:
- No requiere Open3D.
- Usa scipy.cKDTree si está disponible.
"""
...
def top_axis_subset(V: np.ndarray, axis: int = 2, q: float = 0.45) -> np.ndarray:
    """
    Selecciona zona dental de la malla para evitar que la base domine.
    En maxilar superior suele funcionar z >= quantile(z, q).
    """
    V = finite_points(V)
...
def target_foreground_subset(X: np.ndarray, labels: Optional[np.ndarray]) -> np.ndarray:
    """
    Usa dientes/foreground como ancla.
    Si no hay suficientes puntos, usa toda la nube.
    """
    X = finite_points(X)
...
def target_focus_subset(X: np.ndarray,
    ...
    """
    Subset todavía más clínico:
    usa d21 + vecinos si existen suficientes puntos.
    Si no, cae a foreground.
    """
    X = finite_points(X)
...
def dental_bbox_transform(V_all, V_anchor, X_anchor, axiswise=False):
    """
    Coarse bbox alignment.
    axiswise=False: escala uniforme por diagonal robusta.
    axiswise=True: escala por eje.
    """
...
def dental_pca_transform(V_all, V_anchor, X_anchor):
    ...
    PCA similarity:
    - source = malla dental
    - target = nube foreground/focus
    - prueba signos de PCA
    - escoge menor distancia NN
    """
    V_anchor = finite_points(V_anchor)
    X_anchor = finite_points(X_anchor)
    V_all = finite_points(V_all)
...
def icp_refine_mesh_to_cloud(
    ...
    """
    ICP point-to-point restringido a zona dental.

    Source: V_anchor_aligned, proveniente de malla raw ya coarse-aligned.
    Target: X_anchor, nube 8192 foreground/focus.

    Se aplica la transformación acumulada sobre V_all_aligned.
    """
    meta = {
        "enabled": False,
    ...
    for it in range(int(n_iter)):
        ...
        # max correspondence dinámico:
        # se usa el mínimo entre max_corr y quantile robusto amplio.
        ...
        if mask_corr.sum() < 20:
            # si el umbral fue muy estricto, usa trimming sin max_corr
            mask_corr = np.ones_like(dist_f, dtype=bool)
        ...
        # trimmed ICP: elimina correspondencias peores
...
def refine_translation_nn(V_all_aligned, V_anchor_aligned, X_anchor):
    """
    Modo legacy: solo traslación robusta.
    Se deja por compatibilidad, pero v13 recomienda ICP.
    """
    if not HAS_SCIPY:
...
def align_raw_mesh_dental(
    ...
    """
    Alinea malla raw al sistema de la nube X.

    Cambio central v13:
    - dental_pca_refine ahora hace:
        PCA coarse + ICP dental fino
      en vez de solo traslación NN.
    """
    ...
    # ------------------------------------------------------------
    # Modos nuevos / refinados con ICP
    # ------------------------------------------------------------
    ...
    if align_mode in {"dental_pca_refine", "dental_pca_icp"}:
        # En v13 dental_pca_refine ahora usa ICP real
...
def labels_to_colors(labels, alpha_bg=0.55, alpha_fg=1.0):
    labels = np.asarray(labels).reshape(-1).astype(np.int64)
    out = []
    for y in labels:
        if int(y) == 0:
            out.append(hex_to_rgba01("#000000", alpha_bg))
        else:
            out.append(hex_to_rgba01(CLASS_COLORS.get(int(y), "#444444"), alpha_fg))
    return out


def error_colors_all(pred, gt, alpha_ok=0.16):
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
...
def neighbor_focus_colors(labels, d21_class, neighbors, alpha_bg=0.14):
    labels = np.asarray(labels).reshape(-1).astype(np.int64)
...
def d21_tp_error_colors(pred, gt, d21_class, neighbors, alpha_bg=0.12):
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
    gt = np.asarray(gt).reshape(-1).astype(np.int64)
    n = min(len(pred), len(gt))
    pred, gt = pred[:n], gt[:n]
    ...
    for p, g in zip(pred, gt):
        ...
        if is_tp:
            ...
            out.append(hex_to_rgba01(COLOR_LEFT_NEIGH, 0.95))
        elif len(neigh_ids) >= 2 and g == neigh_ids[1]:
            out.append(hex_to_rgba01(COLOR_RIGHT_NEIGH, 0.95))
        elif g == 0:
            out.append(hex_to_rgba01("#000000", alpha_bg))
        else:
...
def make_summary_png(data, out_png, d21_class, neighbors, elev, azim,
    ...
    fig.suptitle(title or "Vista paper — alineación dental + nubes coloreadas",
    ...
    fig.text(
        0.5, 0.012,
        "Alineación v13: coarse dental + ICP restringido; background en negro; malla translúcida como referencia.",
    ...
    for cls in classes:
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers",
                marker=dict(size=float(point_size), color=color_c, opacity=1.0),
...
def add_plotly_error_points(fig, row, col, X, pred, gt, point_size):
    ...
    traces = [
        ("errores: bg correcto", bg_ok, "rgba(0,0,0,0.16)", 1.0),
        ("errores: correcto", non_bg_ok, "rgba(190,190,190,0.18)", 1.0),
        ("errores: pred ≠ GT", err, COLOR_ERROR, 1.08),
    ]

    for name, mask, color, size_mult in traces:
        if not np.any(mask):
            continue
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers",
                marker=dict(size=float(point_size * size_mult), color=color),
                name=name,
                showlegend=True,
            ),
            row=row, col=col,
...
def add_plotly_d21_error_points(fig, row, col, X, pred, gt, d21_class, neighbors, point_size):
    ...
    if len(neigh_ids) >= 1:
        color_map[f"vecino izq. {neigh_ids[0]}"] = COLOR_LEFT_NEIGH
    if len(neigh_ids) >= 2:
        color_map[f"vecino der. {neigh_ids[1]}"] = COLOR_RIGHT_NEIGH
    ...
    for name, mask in masks.items():
        ...
        fig.add_trace(
            go.Scatter3d(
                x=X[mask, 0], y=X[mask, 1], z=X[mask, 2],
                mode="markers",
                marker=dict(size=float(point_size * 1.08), color=color_map.get(name, COLOR_OTHER)),
...
def make_plotly_html(data, out_html, d21_class, neighbors, mesh_opacity,
    ...
    add_plotly_points_by_class(fig, 2, 1, X, labels["gt_focus"], colors["gt_focus"], "GT_focus", point_size * 1.08, show_classes)
    add_plotly_points_by_class(fig, 2, 2, X, labels["pred_focus"], colors["pred_focus"], "Pred_focus", point_size * 1.08, show_classes)
...
def main():
    ...
    ap.add_argument(
        ...
        help="foreground usa todos los dientes; d21_neighbors usa d21 + vecinos como ancla ICP.",
    )
    ...
    if not args.no_individual:
        out_ind = Path(args.out_individual_dir).resolve() if args.out_individual_dir else out_png.parent / "individual_panels_v13"
