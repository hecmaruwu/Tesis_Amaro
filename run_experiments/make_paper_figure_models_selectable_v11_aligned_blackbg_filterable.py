...
"""
make_paper_figure_models_selectable_v11_aligned_blackbg_filterable.py

Objetivo:
- Mantener la alineación base tipo v5: raw mesh -> espacio de xyz_labels.npy.
- Agregar refinamiento automático de traslación para que la nube 8192 calce mejor sobre la malla.
- Clase 0/background en negro.
- Plotly filtrable por clase: cada clase queda como trace separada.
- Colores foco:
    d21 = naranja
...
# ============================================================
# ALIGNMENT — base v5 + refinamiento automático
# ============================================================
...
def refine_alignment_nearest_translation(
    V_aligned: np.ndarray,
    X: np.ndarray,
    labels: Optional[np.ndarray] = None,
    use_fg_only: bool = True,
    max_points: int = 4096,
) -> Tuple[np.ndarray, Dict]:
    """
    Refinamiento simple y estable:
    1. Toma puntos de la nube.
    2. Busca el vértice de malla más cercano.
    3. Calcula la mediana de (punto - vértice_cercano).
    4. Traslada la malla con esa mediana.

    Esto corrige desfases residuales sin deformar la malla.
    """
    if not HAS_SCIPY:
        return V_aligned, {"enabled": False, "reason": "scipy_not_available"}

    V = finite_points(V_aligned)
    X = finite_points(X)

    if labels is not None and use_fg_only:
        labels = np.asarray(labels).reshape(-1).astype(np.int64)
        n = min(X.shape[0], labels.shape[0])
        X_use = X[:n][labels[:n] != 0]
    else:
        X_use = X
    ...
    if X_use.shape[0] > max_points:
        rng = np.random.default_rng(42)
        idx = rng.choice(X_use.shape[0], size=max_points, replace=False)
        X_use = X_use[idx]

    tree = cKDTree(V)
    _, idx_nn = tree.query(X_use, k=1, workers=-1)

    residual = X_use - V[idx_nn]
    shift = np.median(residual, axis=0).astype(np.float32)

    V2 = V + shift.reshape(1, 3)

    meta = {
        "enabled": True,
        "method": "nearest_median_translation",
        "use_fg_only": bool(use_fg_only),
        "n_points_used": int(X_use.shape[0]),
        "shift_xyz": shift.tolist(),
        "median_abs_residual_before": np.median(np.abs(residual), axis=0).tolist(),
    }

    return V2.astype(np.float32), meta
...
def align_raw_mesh(
    ...
    raw_faces,
    target_xyz,
    align_mode="robust_bbox",
    refine_mode="nearest_median",
    refine_labels=None,
):
    """
    Base igual a v5:
    raw mesh se alinea al espacio de target_xyz con robust_bbox/bbox/unit_sphere.
    Luego opcionalmente se corrige traslación residual.
    """
    ...
    refine_meta = {"enabled": False, "mode": refine_mode}

    if refine_mode == "none":
        pass
    elif refine_mode == "nearest_median":
        V, refine_meta = refine_alignment_nearest_translation(
            V_aligned=V,
            X=target_xyz,
            labels=refine_labels,
            use_fg_only=True,
        )
    elif refine_mode == "nearest_median_all":
        V, refine_meta = refine_alignment_nearest_translation(
            V_aligned=V,
            X=target_xyz,
            labels=refine_labels,
        ...
        raise ValueError(f"refine_mode no reconocido: {refine_mode}")

    meta["refine"] = refine_meta
    return V.astype(np.float32), raw_faces, meta
...
def labels_to_colors(labels, alpha_bg=0.58, alpha_fg=1.0):
    """
    Clase 0/background en negro con alpha configurable.
    """
    labels = np.asarray(labels).reshape(-1).astype(np.int64)
    out = []
...
def error_colors_all(pred, gt, alpha_ok=0.18):
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
    ...
    for p, g in zip(pred, gt):
        if int(p) == int(g):
            if int(g) == 0:
                out.append(hex_to_rgba01("#000000", alpha_ok))
            else:
...
def neighbor_focus_colors(labels, d21_class: int, neighbors: List[Tuple[str, int]], alpha_bg=0.18):
    """
    Mismos colores para GT y Pred:
    d21 = naranja
    vecino izquierdo = azul
    vecino derecho = rosado
    background = negro
    ...
    for y in labels:
        ...
        if yy == int(d21_class):
            ...
            out.append(hex_to_rgba01(COLOR_OTHER, 0.10))

    return out


def d21_tp_error_colors(pred, gt, d21_class: int, neighbors: List[Tuple[str, int]], alpha_bg=0.16):
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
    gt = np.asarray(gt).reshape(-1).astype(np.int64)
    ...
    for p, g in zip(pred, gt):
        ...
        if is_tp:
            ...
            out.append(hex_to_rgba01(COLOR_OTHER, 0.08))
...
def prepare_case_data(case_dir, model_key, d21_class, neighbors, align_mode, refine_mode):
    case_dir = Path(case_dir)
    ...
    colors = {
        "gt_all": labels_to_colors(gt, alpha_bg=0.50, alpha_fg=1.0),
        "pred_all": labels_to_colors(pred, alpha_bg=0.50, alpha_fg=1.0),
        "err_all": error_colors_all(pred, gt, alpha_ok=0.18),
        "gt_focus": neighbor_focus_colors(gt, d21_class, neighbors, alpha_bg=0.12),
        "pred_focus": neighbor_focus_colors(pred, d21_class, neighbors, alpha_bg=0.12),
        "d21_error": d21_tp_error_colors(pred, gt, d21_class, neighbors, alpha_bg=0.12),
    }
...
def add_mesh_static(ax, V, F, mesh_alpha=0.24):
    ...
    poly = Poly3DCollection(
        ...
        edgecolors=(0.42, 0.42, 0.42, 0.026),
        linewidths=0.010,
        antialiased=True,
    )
...
def make_summary_png(
    ...
    for i, (lab, col) in enumerate(legend_items):
        ...
        fig.text(x + 0.017, y0, lab, color="black", fontsize=9.0, ha="left", va="center")

    fig.text(
        0.5,
        0.012,
        "Malla raw alineada y translúcida como referencia; background en negro; puntos 8192 coloreados por clase/predicción/error.",
        ha="center",
        fontsize=9.5,
        color="#333333",
    )
...
def add_plotly_error_points(fig, row, col, X, pred, gt, point_size):
    ...
    if np.any(bg_ok):
        fig.add_trace(
            go.Scatter3d(
                x=X[bg_ok, 0], y=X[bg_ok, 1], z=X[bg_ok, 2],
                mode="markers",
                marker=dict(size=float(point_size), color="rgba(0,0,0,0.18)"),
                name="errores: bg correcto",
                showlegend=True,
            ),
            row=row, col=col,
        )
    ...
    if np.any(non_bg_ok):
        fig.add_trace(
            go.Scatter3d(
                x=X[non_bg_ok, 0], y=X[non_bg_ok, 1], z=X[non_bg_ok, 2],
                mode="markers",
                marker=dict(size=float(point_size), color="rgba(190,190,190,0.18)"),
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                marker=dict(
                    ...
                    color=color_map.get(name, COLOR_OTHER),
                    opacity=1.0,
                ),
...
def main():
    ...
    ap.add_argument(
        ...
        choices=["none", "nearest_median", "nearest_median_all"],
        help="Corrige desfase residual de la malla después de robust_bbox.",
    )
    ...
    ap.add_argument("--point_size", type=float, default=5.4)
    ap.add_argument("--mesh_alpha", type=float, default=0.24)

    ap.add_argument("--plotly_mesh_opacity", type=float, default=0.20)
    ap.add_argument("--plotly_point_size", type=float, default=2.35)
    ...
    print("[INFO] refine_mode:", args.refine_mode)
    ...
    if not args.no_individual:
        if args.out_individual_dir:
            ...
            out_ind = out_png.parent / "individual_panels_v11"
