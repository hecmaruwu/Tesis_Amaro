...
"""
build_best_model_comparison_cases_v3.py

Versión corregida del empaquetador de casos.

Mejora principal respecto a v1/v2:
- Copia también predicciones por punto desde:
    <inference_dir>/predictions/test_rowX_SAMPLE_pred.npy
    <inference_dir>/predictions/test_rowX_SAMPLE_gt.npy
    <inference_dir>/predictions/test_rowX_SAMPLE_xyz.npy
- Las copia dentro del case como:
    case_dir/<modelo>/pred_labels.npy
    case_dir/<modelo>/gt_labels.npy
    case_dir/<modelo>/xyz_labels.npy
- También soporta manifests que ya tengan columnas pred_npy/gt_npy/xyz_npy.

Esto permite que make_paper_figure_models_selectable_v2/v3 use render 3D homogéneo,
en vez de PNG fallback.
"""
...
def build_sample_jaw_to_row(rows: List[Dict[str, str]]) -> Dict[Tuple[str, str], int]:
    """
    Mapea (sample_name, jaw) -> posición LOCAL dentro de X_split/Y_split.

    Importante:
    index_test.csv puede tener row_i original/global, por ejemplo 401,
    pero X_test.npz tiene posiciones locales 0..94.
    Por eso aquí usamos enumerate(rows), no row_i.
    """
    out = {}
    if not rows:
        return out

    for local_pos, r in enumerate(rows):
        out[key_from_row(r)] = int(local_pos)
...
def _candidate_prediction_paths(
    ...
    """
    kind: pred | gt | xyz
    Busca primero en manifest, luego patrones estándar.
    """
    inference_dir = Path(inference_dir)
    sample_safe = sanitize(sample_name)
    tag = f"{split}_row{int(row_i)}_{sample_safe}" if sample_safe else f"{split}_row{int(row_i)}"

    key_map = {
        "pred": ["pred_npy", "prediction_npy", "pred_labels", "pred_path"],
        "gt": ["gt_npy", "y_npy", "label_npy", "gt_labels"],
        "xyz": ["xyz_npy", "x_npy", "points_npy", "xyz_path"],
    }
    suffix_map = {
        "pred": "pred",
        "gt": "gt",
    ...
    for k in key_map.get(kind, []):
        rel = manifest_entry.get(k, "") if manifest_entry else ""
        if rel:
            p = Path(rel)
    ...
    # 2) Patrones estándar del exportador nuevo
    suffix = suffix_map[kind]
    cands.extend([
        inference_dir / "predictions" / f"{tag}_{suffix}.npy",
        inference_dir / "predictions" / f"{tag}_{suffix}.npz",
        inference_dir / "predictions" / f"{split}_row{int(row_i)}_{sample_safe}_{suffix}.npy",
        inference_dir / "predictions" / f"{split}_row{int(row_i)}_{sample_safe}_{suffix}.npz",
        inference_dir / "predictions" / f"row_{int(row_i)}_{suffix}.npy",
        inference_dir / "predictions" / f"row_{int(row_i)}_{suffix}.npz",
        inference_dir / "predictions" / f"{sample_safe}_{suffix}.npy",
        inference_dir / "predictions" / f"{sample_safe}_{suffix}.npz",
    ])

    # 3) Fallback amplio, por si el nombre tiene diferencias menores
    pred_dir = inference_dir / "predictions"
    if pred_dir.exists():
        patterns = [
            f"*row{int(row_i)}*{sample_safe}*_{suffix}.npy",
            f"*row{int(row_i)}*{sample_safe}*_{suffix}.npz",
            f"*row{int(row_i)}*{suffix}*.npy",
            f"*row{int(row_i)}*{suffix}*.npz",
        ]
        for pat in patterns:
            cands.extend(sorted(pred_dir.glob(pat)))

    # Quitar duplicados preservando orden
    out = []
    seen = set()
    for p in cands:
        rp = str(Path(p).resolve()) if not Path(p).is_absolute() else str(Path(p))
        if rp not in seen:
            out.append(Path(p))
            seen.add(rp)
    return out
...
def load_prediction_array(path: Path) -> np.ndarray:
    ...
    Si por error viene como dtype object, lo convierte a ndarray numérico.
    """
    path = Path(path)
    ...
    if isinstance(arr, np.lib.npyio.NpzFile):
        keys = list(arr.keys())
        preferred = ["pred", "prediction", "pred_labels", "labels", "y", "gt", "xyz", "points", "X", "Y"]
        key = None
        for k in preferred:
            if k in keys:
                key = k
...
def copy_prediction_arrays(
    ...
    """
    Copia pred/gt/xyz si existen.
    Devuelve status con rutas de origen.
    """
    status = {
        "pred": False,
        "gt": False,
    ...
    specs = {
        "pred": ("pred_labels.npy", np.int64),
        "gt": ("gt_labels.npy", np.int64),
        "xyz": ("xyz_labels.npy", np.float32),
    }

    for kind, (dst_name, dtype) in specs.items():
        found = None
        for cand in _candidate_prediction_paths(inference_dir, manifest_entry, row_i, sample_name, split, kind):
            if Path(cand).exists():
                found = Path(cand)
                break
        ...
        try:
            arr = load_prediction_array(found)
            if kind in ("pred", "gt"):
                arr = arr.reshape(-1).astype(dtype)
            else:
                arr = arr.astype(dtype)
            np.save(dst_dir / dst_name, arr)
            status[kind] = True
            status[f"{kind}_src"] = str(found)
        except Exception as e:
            status[kind] = False
            status[f"{kind}_src"] = f"ERROR loading {found}: {type(e).__name__}: {e}"
...
def main():
    ...
    for row_i in rows_to_use:
        ...
        for model_name, inf_dir in inf_dirs.items():
            ...
            png_status = copy_inference_pngs(mani, inf_dir, model_dir) if mani else {"all": False, "errors": False, "d21": False}
            pred_status = copy_prediction_arrays(
                manifest_entry=mani,
                inference_dir=inf_dir,
                dst_dir=model_dir,
                row_i=row_i,
            ...
            model_copy_status[model_name] = {
                "png": png_status,
                "arrays": pred_status,
                "has_pred_labels": bool((model_dir / "pred_labels.npy").exists()),
                "has_gt_labels": bool((model_dir / "gt_labels.npy").exists()),
                "has_xyz_labels": bool((model_dir / "xyz_labels.npy").exists()),
            }
