...
"""
make_paper_figure_models_selectable_v14_dental_icp_ivory_dense.py

Versión v14:
- Mantiene las mismas vistas/salidas de v13:
    PNG resumen 2x3
    HTML Plotly filtrable
    PNG individuales
    summary.json

- Mantiene alineación dental v13:
...
- Mejora visual:
    - puntos más densos por defecto
    - puntos secundarios en marfil mate/beige suave
    - malla un poco más transparente
    - vistas d21 + vecinos más legibles
    - colores menos “blancos perdidos” contra fondo blanco
...
# Marfil mate: suficientemente distinto del fondo blanco, sin verse amarillo feo.
COLOR_OTHER = "#D8D0C2"
COLOR_CORRECT_TOOTH = "#D8D0C2"
...
def error_colors_all(pred, gt, alpha_ok=0.24):
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
    ...
    for p, g in zip(pred, gt):
        if int(p) == int(g):
            if int(g) == 0:
                out.append(hex_to_rgba01(COLOR_CORRECT_BG, 0.18))
            else:
...
def neighbor_focus_colors(labels, d21_class, neighbors, alpha_bg=0.14):
    ...
    for y in labels:
        ...
        if y == int(d21_class):
            ...
            out.append(hex_to_rgba01(COLOR_OTHER, 0.24))
    return out
...
def d21_tp_error_colors(pred, gt, d21_class, neighbors, alpha_bg=0.12):
    ...
    for p, g in zip(pred, gt):
        ...
        if is_tp:
            ...
            out.append(hex_to_rgba01(COLOR_OTHER, 0.22))
    return out
...
def add_mesh_static(ax, V, F, mesh_alpha=0.18):
    ...
    poly = Poly3DCollection(
        ...
        edgecolors=(0.42, 0.42, 0.42, 0.020),
        linewidths=0.008,
        antialiased=True,
    )
...
def make_summary_png(data, out_png, d21_class, neighbors, elev, azim,
    ...
    panel_specs = [
        ...
        ("GT: d21 + vecinos\nazul/naranja/rosado", "gt_focus", zoom_focus, point_size * 1.16),
        (f"{pretty}: d21 + vecinos\nazul/naranja/rosado", "pred_focus", zoom_focus, point_size * 1.16),
        ("Error d21\nverde=TP | rojo=FP/FN", "d21_error", zoom_focus, point_size * 1.20),
    ]
    ...
    fig.text(
        0.5, 0.012,
        "Alineación v14: coarse dental + ICP restringido; puntos secundarios en marfil mate; malla translúcida.",
...
def add_plotly_error_points(fig, row, col, X, pred, gt, point_size):
    ...
    traces = [
        ("errores: bg correcto", bg_ok, "rgba(0,0,0,0.18)", 1.0),
        ("errores: correcto", non_bg_ok, "rgba(216,208,194,0.30)", 1.0),
        ("errores: pred ≠ GT", err, COLOR_ERROR, 1.10),
    ]
...
def add_plotly_d21_error_points(fig, row, col, X, pred, gt, d21_class, neighbors, point_size):
    ...
    color_map = {
        ...
        "otros dientes": "rgba(216,208,194,0.22)",
        "TP d21": COLOR_TP,
    ...
    draw_order = ["background", "otros dientes"]
    if len(neigh_ids) >= 1:
        draw_order.append(f"vecino izq. {neigh_ids[0]}")
    if len(neigh_ids) >= 2:
        draw_order.append(f"vecino der. {neigh_ids[1]}")
    draw_order += ["TP d21", "FP/FN d21"]
...
def main():
    ...
    ap.add_argument("--mesh_alpha", type=float, default=0.18)

    ap.add_argument("--plotly_mesh_opacity", type=float, default=0.16)
    ap.add_argument("--plotly_point_size", type=float, default=4.0)
    ...
    if not args.no_individual:
        out_ind = Path(args.out_individual_dir).resolve() if args.out_individual_dir else out_png.parent / "individual_panels_v14"
