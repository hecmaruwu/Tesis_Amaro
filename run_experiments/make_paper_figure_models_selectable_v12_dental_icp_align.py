...
"""
make_paper_figure_models_selectable_v12_dental_icp_align.py

Versión v12:
- Corrige alineación usando ANCLA DENTAL, no toda la base de la malla.
- Usa foreground labels != 0 como referencia principal.
- Soporta alineación:
    dental_bbox
    dental_pca
    dental_pca_refine
- Clase 0/background en negro.
- Plotly con traces por clase para activar/desactivar dientes.
- PNG resumen 2x3.
- PNG individuales.
"""
...
def top_axis_subset(V: np.ndarray, axis: int = 2, q: float = 0.45) -> np.ndarray:
    """
    Selecciona la zona superior de la malla para evitar que la base domine la alineación.
    Por defecto mantiene vertices con z >= quantile(z, 0.45).
    """
    V = finite_points(V)
...
def score_alignment_nn(source_aligned: np.ndarray, target: np.ndarray) -> float:
    ...
    source_aligned = random_sample_points(source_aligned, max_points=4096)
    target = random_sample_points(target, max_points=4096)
...
def dental_bbox_transform(V_all, V_anchor, X_anchor):
...
def dental_pca_transform(V_all, V_anchor, X_anchor):
    """
    PCA similarity:
    - usa ancla dental de la malla
    - usa foreground de la nube
    - prueba combinaciones de signos para evitar inversión
    - elige la de menor distancia NN
    """
    ...
    for S in sign_options:
        ...
        # evitar reflexión extrema si se quiere conservar orientación;
        # igual se permite si da mucho mejor score.
...
def align_raw_mesh_dental(
    ...
    if align_mode == "robust_bbox":
        V_out, m = dental_bbox_transform(V_all, V_all, X)
        meta.update(m)
        return V_out, raw_faces, meta

    if align_mode == "dental_bbox":
        V_out, m = dental_bbox_transform(V_all, V_anchor, X_anchor)
        meta.update(m)
        return V_out, raw_faces, meta
    ...
    if align_mode == "dental_pca_refine":
        V_pca, m = dental_pca_transform(V_all, V_anchor, X_anchor)

        # Recalcular ancla transformada con el mismo transform aplicado:
        # se aproxima tomando la selección superior ya transformada por índice indirecto no trivial.
        # Para estabilidad, usamos V_pca top subset en el espacio transformado.
        V_anchor_aligned = top_axis_subset(V_pca, axis=mesh_top_axis, q=mesh_top_q)

        V_out, r = refine_translation_nn(V_pca, V_anchor_aligned, X_anchor)
        meta.update(m)
        meta["refine"] = r
        return V_out, raw_faces, meta
...
def colors_to_plotly(colors):
    out = []
    for c in colors:
        out.append(c if isinstance(c, str) else rgba_to_plotly(c))
    return out
...
def prepare_case_data(case_dir, model_key, d21_class, neighbors, align_mode, mesh_top_q):
    case_dir = Path(case_dir)
...
def make_summary_png(data, out_png, d21_class, neighbors, elev, azim,
    ...
    fig.text(
        0.5, 0.012,
        "Alineación basada en región dental; background en negro; malla translúcida como referencia.",
...
def main():
    ...
    ap.add_argument(
        ...
        choices=["none", "robust_bbox", "dental_bbox", "dental_pca", "dental_pca_refine"],
    )
    ...
    if not args.no_individual:
        out_ind = Path(args.out_individual_dir).resolve() if args.out_individual_dir else out_png.parent / "individual_panels_v12"
