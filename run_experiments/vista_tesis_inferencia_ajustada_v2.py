...
"""
vista_tesis_inferencia_ajustada_v2.py

Visualizaciones cualitativas 2x3 para tesis dental 3D.

V2 corrige y estandariza:
...
def build_panel_figure(xyz: np.ndarray,
                       ...
                       multiclass_bg_mode: str) -> go.Figure:
    ...
    colors_gt = colors_for_multiclass(gt, bg_alpha=multiclass_bg_opacity, bg_mode=multiclass_bg_mode)
    colors_pred = colors_for_multiclass(pred, bg_alpha=multiclass_bg_opacity, bg_mode=multiclass_bg_mode)
    ...
    for (trace_name, colors), (row, col) in zip(panel_data, positions):
        if vertices is not None and faces is not None and mesh_opacity > 0:
            fig.add_trace(mesh_trace(vertices, faces, opacity=mesh_opacity), row=row, col=col)
        fig.add_trace(
            point_trace(
                xyz=xyz,
                colors=colors,
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--out_dir",
        default=None,
        help="Carpeta de salida. Default: <base_dir>/VISTA_TESIS_INFERENCIA_AJUSTADA_V2.",
    )
    ap.add_argument(
        "--selected_mesh_root",
    ...
    ap.add_argument("--mesh_opacity", type=float, default=0.12)
...
def main() -> None:
    ...
    out_root = Path(args.out_dir) if args.out_dir else base_dir / "VISTA_TESIS_INFERENCIA_AJUSTADA_V2"
    ensure_dir(out_root)
    ...
    print("=" * 90)
    print("[INFO] vista_tesis_inferencia_ajustada_v2.py")
    ...
    for case_dir in case_dirs:
        ...
        for model in models:
            ...
            for view_name, camera in views.items():
                ...
                summary = {
                    "script": "vista_tesis_inferencia_ajustada_v2.py",
    ...
    manifest_path = out_root / "manifest_vista_tesis_inferencia_ajustada_v2.csv"
    ...
    save_json(run_meta, out_root / "run_meta_vista_tesis_inferencia_ajustada_v2.json")

    print("\n" + "=" * 90)
    print("[DONE] Visualizaciones generadas.")
    print(f"[OK] Manifest: {manifest_path}")
    print(f"[OK] Run meta: {out_root / 'run_meta_vista_tesis_inferencia_ajustada_v2.json'}")
    print("=" * 90)
