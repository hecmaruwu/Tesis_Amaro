...
"""
make_paper_figure_models_selectable_v2.py
...
Ejemplo 1 modelo:
python3 -u make_paper_figure_models_selectable_v2.py \
...
def align_trimesh_to_cloud(mesh_tm: trimesh.Trimesh, xyz_ref: np.ndarray) -> trimesh.Trimesh:
...
def main():
    ...
    if raw_path is not None and raw_path.exists():
        ...
        if tm is not None:
            mesh_pv = trimesh_to_pyvista(align_trimesh_to_cloud(tm, xyz))
    ...
    for model_key in selected_models:
        ...
        if not pred_path.exists():
            ...
            continue
        pred = np.load(pred_path, allow_pickle=True).reshape(-1).astype(np.int64)
