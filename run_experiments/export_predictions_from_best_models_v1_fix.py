...
class NPZInferDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        # FIX: usar as_tensor sobre copias contiguas evita el error raro
        # TypeError: expected np.ndarray (got numpy.ndarray)
        # que puede aparecer por incompatibilidad NumPy/Torch al importar módulos externos.
        return torch.as_tensor(x.copy(), dtype=torch.float32), torch.as_tensor(y.copy(), dtype=torch.long), torch.tensor(int(i), dtype=torch.long)
