...
def write_plotly_pointcloud(xyz, labels, out_html, title, name="points", point_size=3, max_points=8192):
    """
    V2: Plotly Express con leyenda por clase/diente.
    """
    ...
    df = pd.DataFrame({
        ...
        "y": xyz[:, 1],
        "z": xyz[:, 2],
        "clase": [label_name_from_id(v) for v in labels],
        "id_clase": labels,
    })
    ...
    fig = px.scatter_3d(
        ...
        z="z",
        color="clase",
        hover_data={"id_clase": True, "x": ":.4f", "y": ":.4f", "z": ":.4f"},
        title=title,
    )
    fig.update_traces(marker=dict(size=point_size, opacity=0.95))
    fig.update_layout(**make_plotly_layout(title))
    fig.write_html(str(out_html), include_plotlyjs="cdn")
    return True
...
def write_plotly_raw_plus_points(mesh_tm, xyz, labels, out_html, title, point_size=3):
    """
    V2: raw mesh en gris + puntos por clase con leyenda separada.
    """
    if go is None:
        return False
    ...
    for cls in sorted(np.unique(labels).tolist()):
        mask = labels == cls
        pts = xyz[mask]
        color = mcolors.to_hex(cmap(int(cls) % 20))
        fig.add_trace(go.Scatter3d(
            x=pts[:, 0],
            y=pts[:, 1],
            z=pts[:, 2],
            mode="markers",
            marker=dict(size=point_size, color=color, opacity=0.95),
...
def write_plotly_errors(xyz, err_mask, out_html, title):
    ...
    df = pd.DataFrame({
        ...
        "y": xyz[:, 1],
        "z": xyz[:, 2],
        "estado": np.where(err_mask, "error", "correcto"),
    })
    ...
    fig = px.scatter_3d(
        ...
        z="z",
        color="estado",
        color_discrete_map={"error": "red", "correcto": "lightgray"},
        title=title,
    )
...
def write_plotly_binary(xyz, mask, out_html, title):
    """
    V2: d21 vs resto/fondo con leyenda explícita.
    """
    if px is None or pd is None:
        return False
    ...
    df = pd.DataFrame({
        ...
        "y": xyz[:, 1],
        "z": xyz[:, 2],
        "clase": np.where(mask, "d21", "resto/fondo"),
    })
    ...
    fig = px.scatter_3d(
        ...
        z="z",
        color="clase",
        color_discrete_map={"d21": "red", "resto/fondo": "lightgray"},
        title=title,
    )
    fig.update_traces(marker=dict(size=3, opacity=0.95))
    fig.update_layout(**make_plotly_layout(title))
    fig.write_html(str(out_html), include_plotlyjs="cdn")
    return True
