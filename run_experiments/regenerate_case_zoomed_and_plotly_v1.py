...
# -*- coding: utf-8 -*-
"""
regenerate_case_zoomed_and_plotly_v1.py

Post-procesa una carpeta de caso generada por build_best_model_comparison_cases_v1/v2.py:
...
def tab20_hex(labels):
    import matplotlib.cm as cm, matplotlib.colors as mcolors
    cmap = cm.get_cmap('tab20', 20)
    return [mcolors.to_hex(cmap(int(v) % 20)) for v in np.asarray(labels).reshape(-1)]


def binary_hex(mask, pos='#e052aa', neg='#9a9a9a'):
    return [pos if bool(v) else neg for v in np.asarray(mask).reshape(-1)]


def layout(title):
    return dict(title=title, scene=dict(xaxis=dict(visible=False), yaxis=dict(visible=False), zaxis=dict(visible=False), aspectmode='data'), margin=dict(l=0,r=0,t=45,b=0), showlegend=True)


def write_raw_mesh_html(mesh_tm, out_html: Path, title: str):
    if go is None or mesh_tm is None or mesh_tm.faces is None: return False
    V = np.asarray(mesh_tm.vertices); F = np.asarray(mesh_tm.faces)
    fig = go.Figure(go.Mesh3d(x=V[:,0], y=V[:,1], z=V[:,2], i=F[:,0], j=F[:,1], k=F[:,2], color='lightgray', opacity=1.0, name='raw mesh'))
    fig.update_layout(**layout(title)); fig.write_html(str(out_html), include_plotlyjs='cdn'); return True


def write_points_html(xyz, labels, out_html: Path, title: str, name='points', point_size=4):
    if go is None: return False
    xyz = np.asarray(xyz, np.float32); labels = np.asarray(labels).reshape(-1)
    fig = go.Figure(go.Scatter3d(x=xyz[:,0], y=xyz[:,1], z=xyz[:,2], mode='markers', marker=dict(size=point_size, color=tab20_hex(labels), opacity=0.95), name=name))
    fig.update_layout(**layout(title)); fig.write_html(str(out_html), include_plotlyjs='cdn'); return True


def write_raw_plus_points_html(mesh_tm, xyz, labels, out_html: Path, title: str, point_size=4):
    if go is None or mesh_tm is None or mesh_tm.faces is None: return False
    V = np.asarray(mesh_tm.vertices); F = np.asarray(mesh_tm.faces); xyz = np.asarray(xyz, np.float32)
    fig = go.Figure()
    fig.add_trace(go.Mesh3d(x=V[:,0], y=V[:,1], z=V[:,2], i=F[:,0], j=F[:,1], k=F[:,2], color='lightgray', opacity=0.25, name='raw mesh'))
    fig.add_trace(go.Scatter3d(x=xyz[:,0], y=xyz[:,1], z=xyz[:,2], mode='markers', marker=dict(size=point_size, color=tab20_hex(labels), opacity=0.95), name='points'))
    fig.update_layout(**layout(title)); fig.write_html(str(out_html), include_plotlyjs='cdn'); return True


def write_errors_html(xyz, err_mask, out_html: Path, title: str):
    if go is None: return False
    xyz = np.asarray(xyz, np.float32); err_mask = np.asarray(err_mask).reshape(-1).astype(bool)
    fig = go.Figure()
    fig.add_trace(go.Scatter3d(x=xyz[:,0], y=xyz[:,1], z=xyz[:,2], mode='markers', marker=dict(size=2, color='lightgray', opacity=0.12), name='all'))
    if err_mask.any():
        xe = xyz[err_mask]
        fig.add_trace(go.Scatter3d(x=xe[:,0], y=xe[:,1], z=xe[:,2], mode='markers', marker=dict(size=5, color='red', opacity=1.0), name='errors'))
    fig.update_layout(**layout(title)); fig.write_html(str(out_html), include_plotlyjs='cdn'); return True


def write_binary_html(xyz, mask, out_html: Path, title: str):
    if go is None: return False
    xyz = np.asarray(xyz, np.float32)
    fig = go.Figure(go.Scatter3d(x=xyz[:,0], y=xyz[:,1], z=xyz[:,2], mode='markers', marker=dict(size=4, color=binary_hex(mask), opacity=0.95), name='d21 vs bg'))
    fig.update_layout(**layout(title)); fig.write_html(str(out_html), include_plotlyjs='cdn'); return True
