...
"""
make_paper_figure_models_selectable_v15_dental_icp_beige_dense.py

Versión v15:
- Mantiene las mismas vistas/salidas de v13/v14:
    PNG resumen 2x3
...
- Mejora visual v15:
    ...
    - beige mate más visible (#C2B8A3)
    - malla más transparente
    - leyenda Plotly mucho más grande, ancha y alta
    - layout HTML más ancho para que la leyenda no corte etiquetas
...
# Beige mate visible, más parecido al tono que querías:
# no es blanco, no es amarillo fuerte.
COLOR_OTHER = "#C2B8A3"
COLOR_CORRECT_TOOTH = "#C2B8A3"
...
def error_colors_all(pred, gt, alpha_ok=0.42):
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
    ...
    for p, g in zip(pred, gt):
        if int(p) == int(g):
            if int(g) == 0:
                out.append(hex_to_rgba01(COLOR_CORRECT_BG, 0.20))
            else:
...
def neighbor_focus_colors(labels, d21_class, neighbors, alpha_bg=0.16):
    labels = np.asarray(labels).reshape(-1).astype(np.int64)
    ...
    for y in labels:
        ...
        if y == int(d21_class):
            ...
            out.append(hex_to_rgba01(COLOR_OTHER, 0.38))

    return out


def d21_tp_error_colors(pred, gt, d21_class, neighbors, alpha_bg=0.14):
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
    gt = np.asarray(gt).reshape(-1).astype(np.int64)
    ...
    for p, g in zip(pred, gt):
        ...
        if p == d21_class and g == d21_class:
            ...
            out.append(hex_to_rgba01(COLOR_OTHER, 0.35))
...
def add_mesh_static(ax, V, F, mesh_alpha=0.16):
...
def add_points_static(ax, X, colors, point_size=9.2):
    X = finite_points(X)
...
def make_summary_png(data, out_png, d21_class, neighbors, elev, azim,
    ...
    fig.text(
        0.5,
        0.012,
        "Alineación v15: coarse dental + ICP restringido; beige mate visible; malla translúcida.",
        ha="center",
        fontsize=9.5,
        color="#333333",
    )
...
def add_plotly_error_points(fig, row, col, X, pred, gt, point_size):
    ...
    traces = [
        ...
        ("errores: correcto", non_bg_ok, "rgba(194,184,163,0.50)", 1.0),
        ("errores: pred ≠ GT", err, COLOR_ERROR, 1.12),
    ]
    ...
    for name, mask, color, size_mult in traces:
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers",
                marker=dict(
                    size=float(point_size * size_mult),
                    color=color,
                    opacity=1.0,
                ),
                name=name,
...
def add_plotly_d21_error_points(fig, row, col, X, pred, gt, d21_class, neighbors, point_size):
    ...
    color_map = {
        ...
        "otros dientes": "rgba(194,184,163,0.46)",
        "TP d21": COLOR_TP,
...
def make_plotly_html(data, out_html, d21_class, neighbors, mesh_opacity,
    ...
    # Leyenda grande y extendida:
    # - x más cerca del gráfico
    # - ancho disponible gracias a width mayor
    # - itemsize grande
    # - font mayor
    # - itemwidth aumentado
    fig.update_layout(
        ...
        margin=dict(l=5, r=430, t=80, b=5),
        legend=dict(
            x=1.015,
            y=0.985,
            xanchor="left",
...
def main():
    ...
    ap.add_argument("--point_size", type=float, default=9.2)
    ap.add_argument("--mesh_alpha", type=float, default=0.16)

    ap.add_argument("--plotly_mesh_opacity", type=float, default=0.14)
    ap.add_argument("--plotly_point_size", type=float, default=4.6)
    ...
    if not args.no_individual:
        out_ind = (
            ...
            else out_png.parent / "individual_panels_v15"
        )
