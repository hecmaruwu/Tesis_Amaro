...
"""
make_paper_figure_models_selectable_v16_dental_icp_warm_beige_solid.py

Versión v16:
- Mantiene las mismas vistas/salidas de v15:
    PNG resumen 2x3
    HTML Plotly filtrable
    PNG individuales
    summary.json

- Mantiene alineación dental v13/v15:
...
- Mejora visual v16:
    ...
    - colores más sólidos, menos efecto acuarela/transparencia
    - background mucho menos dominante en vistas focales
    - malla más tenue para no mezclar colores
    - rojo/verde y dientes focales se dibujan encima
    - Plotly usa trazas por grupos para controlar orden visual
...
Lógica de vistas:
    ...
    Vista 3: errores multiclase
        rojo = pred != GT
        beige = diente correcto
        gris/negro tenue = background correcto
    Vista 4: GT d21 + vecinos
        naranja = d21 real
        azul = d11 real
        rosado = d22 real
        beige = otros dientes reales
        fondo = muy tenue
    Vista 5: Pred d21 + vecinos
        naranja = pred d21
        azul = pred d11
        rosado = pred d22
        beige = pred otros dientes
        fondo = muy tenue
    Vista 6: Error d21
        verde = TP d21
        rojo = FP/FN d21
        azul/rosado = vecinos reales
        beige = otros dientes reales
        fondo = muy tenue
...
# Beige cálido pedido por el usuario.
# Es más visible que #C2B8A3 y no se pierde tanto contra blanco/malla.
COLOR_OTHER = "#E8C39E"
...
# Alphas v16: más sólidos para evitar mezcla tipo acuarela.
ALPHA_BG_MAIN = 0.56
ALPHA_BG_FOCUS = 0.07
ALPHA_BG_ERROR = 0.08
...
def error_colors_all(pred, gt, alpha_ok=ALPHA_OTHER_ERROR):
    """
    Vista 3:
    - rojo = error multiclase pred != GT
    - beige cálido = diente correcto
    - fondo correcto = negro muy tenue
    """
    pred = np.asarray(pred).reshape(-1).astype(np.int64)
...
def neighbor_focus_colors(labels, d21_class, neighbors, alpha_bg=ALPHA_BG_FOCUS):
    ...
    - vecino 1 = azul
    - vecino 2 = rosado
    - otros dientes = beige sólido
    - background = negro casi invisible
    """
    labels = np.asarray(labels).reshape(-1).astype(np.int64)
...
def add_mesh_static(ax, V, F, mesh_alpha=0.08):
...
def plot_static_panel_ordered(ax, V, F, X, pred, gt, key, d21_class, neighbors,
    ...
    """
    Vistas 3/4/5/6 con orden explícito:
    1) malla
    2) fondo tenue
    3) beige
    4) focales
    ...
    if key == "err_all":
        ...
        scatter_by_mask_static(ax, X, bg_ok, COLOR_BG, point_size * 0.80, ALPHA_BG_ERROR)
        scatter_by_mask_static(ax, X, tooth_ok, COLOR_OTHER, point_size * 1.00, ALPHA_OTHER_ERROR)
        scatter_by_mask_static(ax, X, err, COLOR_ERROR, point_size * 1.10, ALPHA_ERROR)
        ...
        if len(neigh_ids) >= 1:
            n1 = labels == neigh_ids[0]
        else:
            n1 = np.zeros_like(bg, dtype=bool)

        if len(neigh_ids) >= 2:
            n2 = labels == neigh_ids[1]
        else:
            n2 = np.zeros_like(bg, dtype=bool)

        other = other & (~d21) & (~n1) & (~n2)

        scatter_by_mask_static(ax, X, bg, COLOR_BG, point_size * 0.65, ALPHA_BG_FOCUS)
        scatter_by_mask_static(ax, X, other, COLOR_OTHER, point_size * 0.95, ALPHA_OTHER_FOCUS)
        scatter_by_mask_static(ax, X, n1, COLOR_LEFT_NEIGH, point_size * 1.12, ALPHA_FOCAL)
        scatter_by_mask_static(ax, X, n2, COLOR_RIGHT_NEIGH, point_size * 1.12, ALPHA_FOCAL)
        ...
        scatter_by_mask_static(ax, X, d21_err, COLOR_ERROR, point_size * 1.22, ALPHA_ERROR)
...
def make_summary_png(data, out_png, d21_class, neighbors, elev, azim,
    ...
    fig.text(
        0.5,
        0.012,
        "Alineación v16: beige cálido sólido; fondo casi invisible en vistas focales; errores/TP encima.",
        ha="center",
        fontsize=9.5,
        color="#333333",
    )
...
def make_plotly_html(data, out_html, d21_class, neighbors, mesh_opacity,
    ...
    # Vista 1 y 2: se mantienen multiclase completas
    add_plotly_points_by_class(
        fig, 1, 1, X, labels["gt_all"], colors["gt_all"],
        "GT", point_size, show_classes,
    )
    add_plotly_points_by_class(
        fig, 1, 2, X, labels["pred_all"], colors["pred_all"],
        "Pred", point_size, show_classes,
    )

    # Vista 3, 4, 5 y 6: orden de trazas explícito, menos mezcla visual
    add_plotly_error_points(
        fig, 1, 3, X, pred, gt, point_size,
    )
    add_plotly_focus_points(
        fig, 2, 1, X, gt, d21_class, neighbors,
        "GT_focus", point_size * 1.08,
    )
    add_plotly_focus_points(
        fig, 2, 2, X, pred, d21_class, neighbors,
        "Pred_focus", point_size * 1.08,
    )
    add_plotly_d21_error_points(
        fig, 2, 3, X, pred, gt, d21_class, neighbors,
        point_size * 1.08,
    )
...
def main():
    ...
    # v16: tamaño grande pero menos sucio que 9.2
    ap.add_argument("--point_size", type=float, default=8.8)

    # v16: malla más tenue para evitar mezcla con beige cálido
    ap.add_argument("--mesh_alpha", type=float, default=0.08)

    ap.add_argument("--plotly_mesh_opacity", type=float, default=0.06)
    ...
    if not args.no_individual:
        out_ind = (
            ...
            else out_png.parent / "individual_panels_v16"
        )
