...
"""
train_pointnetpp_traditional_fixed_v1.py
...
- Métricas:
    * macro: precision/recall/f1/iou (+ acc_macro opcional)
    * global: acc (micro / overall point accuracy)
    * d21 (binario): d21_acc, d21_f1, d21_iou
- Guardado: run_meta.json, history.json, metrics_epoch.csv, test_metrics.json, best.pt, last.pt, plots/
...
def resolve_label_map_path(data_dir: Path, artifacts_dir: Path) -> Optional[Path]:
    """
    Busca label_map.json en:
      1) data_dir/label_map.json
      2) artifacts_dir/label_map.json
    """
    data_dir = Path(data_dir)
    artifacts_dir = Path(artifacts_dir)

    p1 = data_dir / "label_map.json"
    if p1.exists():
        return p1

    p2 = artifacts_dir / "label_map.json"
    if p2.exists():
        return p2
...
def load_label_map(label_map_path: Optional[Path]) -> Tuple[Optional[dict], Optional[dict]]:
...
def infer_num_classes_global(data_dir: Path, artifacts_dir: Optional[Path] = None) -> int:
    """
    1) Si existe label_map.json (data_dir o artifacts_dir), usar max(id2idx)+1
    2) Si no, usar max GLOBAL entre Y_train/Y_val/Y_test
    """
    data_dir = Path(data_dir)
    artifacts_dir = Path(artifacts_dir) if artifacts_dir is not None else data_dir

    lm_path = resolve_label_map_path(data_dir, artifacts_dir)
    if lm_path is not None and lm_path.exists():
        try:
            data = json.load(open(lm_path, "r", encoding="utf-8"))
...
class MetricsBundle:
    """
    Mantiene:
      - Confusion matrix (para fallback)
      - Si torchmetrics está: macro P/R/F1/IoU + acc_macro + acc_micro
    Nota: "acc" (overall) = acc_micro.
    """
    def __init__(self, num_classes: int, device: torch.device, ignore_index: Optional[int] = 0):
        ...
        if self.has_tm:
            # overall / micro
            self._acc_micro = MulticlassAccuracy(
                num_classes=self.num_classes, average="micro", ignore_index=self.ignore
            ).to(device)
            # macro
            self._acc_macro = MulticlassAccuracy(
                num_classes=self.num_classes, average="macro", ignore_index=self.ignore
            ).to(device)
    ...
    def compute(self) -> Dict[str, float]:
        ...
        Retorna:
          - acc: overall/micro
          - acc_macro (si torchmetrics)
          - prec/rec/f1/iou: macro
        """
        if self.has_tm:
            out = {
                "acc": float(self._acc_micro.compute().item()),
        ...
        # fallback con CM (ojo: "acc" es overall/micro)
        cm = self.cm.float()
        tp = torch.diag(cm)
        gt = cm.sum(1)
        pd = cm.sum(0)
...
def colors_d21_mode(y_internal: np.ndarray, d21_internal: Optional[int]) -> np.ndarray:
    """
...
def plot_pointcloud_gt_pred(
    ...
    infer_mode:
      ...
      - errors: TP(d21) verde, FN rojo, FP naranja, resto gris
    """
    ...
    if infer_mode == "d21":
        c_gt = colors_d21_mode(y_gt, d21_internal)
        c_pr = colors_d21_mode(y_pr, d21_internal)
...
class PointNetSetAbstractionSSG(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, points: Optional[torch.Tensor]):
        ...
        new_points = new_points.permute(0, 3, 2, 1).contiguous()  # [B,3+D,nsample,S]
        new_points = self.mlp(new_points)
        new_points = torch.max(new_points, dim=2)[0]              # [B,mlp[-1],S]
        return new_xyz, new_points.transpose(2, 1).contiguous()   # [B,S,mlp[-1]]
...
class PointNetPP_SSG_Seg(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor) -> torch.Tensor:
        ...
        l2_points_new = self.fp3(l2_xyz, l3_xyz, l2_points, l3_points)
...
def run_epoch(model, loader, criterion, optimizer, device,
    ...
    for xyz, y in loader:
        ...
        if torch.any((y < 0) | (y >= num_classes)):
            raise ValueError(
                f"[LABEL ERROR] y fuera de rango: min={int(y.min())}, max={int(y.max())}, num_classes={num_classes}"
            )
    ...
    met = mb.compute()
    out = {"loss": loss_meter / max(1, n_batches), **met}
...
def main():
    ...
    artifacts_dir = Path(args.artifacts_dir) if args.artifacts_dir else (
        data_dir / "artifacts" if (data_dir / "artifacts").exists() else data_dir
    )

    # device
    device = torch.device("cuda") if (args.device == "cuda" and torch.cuda.is_available()) else torch.device("cpu")

    # num_classes (ahora considera artifacts_dir también)
    num_classes = infer_num_classes_global(data_dir, artifacts_dir=artifacts_dir)
    print("[DEBUG] num_classes =", num_classes)

    # ===================== FIX LABEL MAP =====================
    label_map_path = resolve_label_map_path(data_dir, artifacts_dir)
    print("[DEBUG] label_map_path =", str(label_map_path) if label_map_path is not None else "None")

    id2idx, idx2id = load_label_map(label_map_path)
    ...
    run_meta = {
        "script": "train_pointnetpp_traditional_fixed_v1.py",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history["train_acc_macro"].append(tr.get("acc_macro", 0.0)); history["val_acc_macro"].append(va.get("acc_macro", 0.0))
        ...
        with open(csv_path, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow([epoch,"train",tr["loss"], tr["acc"], tr.get("acc_macro", 0.0), tr["prec"], tr["rec"], tr["f1"], tr["iou"],
                        tr["d21_acc"], tr["d21_f1"], tr["d21_iou"], sec])
            w.writerow([epoch,"val",  va["loss"], va["acc"], va.get("acc_macro", 0.0), va["prec"], va["rec"], va["f1"], va["iou"],
                        va["d21_acc"], va["d21_f1"], va["d21_iou"], sec])
        ...
        print(
            ...
            f"train loss={tr['loss']:.4f} acc={tr['acc']:.4f} f1={tr['f1']:.4f} iou={tr['iou']:.4f} | "
            f"val loss={va['loss']:.4f} acc={va['acc']:.4f} f1={va['f1']:.4f} iou={va['iou']:.4f} | "
            f"d21_acc={va['d21_acc']:.4f} d21_f1={va['d21_f1']:.4f} d21_iou={va['d21_iou']:.4f}"
        )
    ...
    save_json(
        {"best_epoch": int(ckpt.get("epoch", best_epoch)), "best_val_f1": float(best_val_f1), "test": te},
