...
Guarda HTML interactivo y PNG estático.
"""
...
# ======================================================
# 🔹 Modelo (debe coincidir con el usado en entrenamiento v7)
# ======================================================
...
class PointNetSeg(nn.Module):
    def __init__(self, k=2):
        ...
        self.conv2 = nn.Sequential(nn.Conv1d(64, 64, 1), nn.BatchNorm1d(64), nn.ReLU())   # pointfeat
...
def plot_inference(points: np.ndarray, preds: np.ndarray, out_html: Path, out_png: Path, pid: str):
    """
    points: (N,3)
    preds : (N,)  1 = resto, 0 = diente 21
    """
    pts_normal = points[preds == 1]
    pts_tooth21 = points[preds == 0]
...
#    fig.write_image(str(out_png))
    print(f"[OK] {pid}: guardado → {out_html.name} / {out_png.name}")
...
def infer_and_visualize(args):
    ...
    # buscar pacientes (por defecto test completo o los primeros N)
    ...
    for sample in tqdm(samples, desc="[INFERENCIA]"):
        ...
        X_tensor = torch.from_numpy(X).unsqueeze(0).to(device)  # (1,N,3)
        with torch.no_grad():
            logits = model(X_tensor)
            preds = logits.argmax(2).cpu().numpy().squeeze()  # (N,)
        plot_inference(
            X,
            preds,
